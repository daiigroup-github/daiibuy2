...
//				$up->partnerId = "org Or wow Id";
    ...
    public function actionForgotPassword() {
        ...
        if (isset($_POST['email'])) {
            ...
            $tomail = $_POST['email'];
            $passwordNewHash = md5($tomail . $password);
            $user = User::model()->find("email = '" . $tomail . "'");
            if (isset($user) && !empty($user)) {
                $user->password = $passwordNewHash;
                if ($user->save()) {
                    $flag = true;
                }
                $documentUrl = "http://" . Yii::app()->request->getServerName() . Yii::app()->baseUrl . "/site/changePassword/?mail=$tomail";
                $username = $user->firstname . ' ' . $user->lastname;
                $emailSend = new EmailSend();
                $emailSend->mailForgetPassword($tomail, $password, $username, $documentUrl);
            } else {
                $messege = "E-mail not found";
    ...
    public function actionChangePassword($mail) {
        ...
        if (isset($_POST['password'])) {
            $user = User::model()->find("email = '" . $mail . "'");
            $oldPassword = md5($mail . $_POST['password']);
            //throw new Exception($oldPassword);
            if ($user->password != $oldPassword) {
                $messege = "Incorect Password";
                $this->render('changePassword', array(
                    'model' => $model,
                    'messege' => $messege));
            } else if ($_POST['newPassword'] != $_POST['confirmNewPassword']) {
                $messege = "Incorect confirm password";
                $this->render('changePassword', array(
                    'model' => $model,
                    'messege' => $messege));
            } else {
                $user->password = md5($mail . $_POST['newPassword']);
                if ($user->save()) {
                    $messege = "Saved";
                    $flag = true;
                    $this->render('changePassword', array(
