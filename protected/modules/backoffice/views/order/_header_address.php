...
//			throw new Exception(print_r($model->status.' ,'.$user->type,true));
            if (($model->status != 3 && ($user->type != 4 && $user->type != 3 && $user->type != 2 && $user->type != 7 )) || ($model->status >= 3 && ($user->type != 4 && $user->type != 3 && $user->type != 7 ))) {
                ?>
...
//					(Yii::app()->controller->action->id == "viewOrder" && $userType <> 1 && $userType <> 5)
                if ((($model->status == 3 || $model->status == 4 ) && ($userType <> 1 && $userType <> 2 && $userType <> 5)) || (Yii::app()->controller->action->id == "view" && ($userType <> 5 && $userType <> 1)) || (Yii::app()->controller->action->id == "print" && $userType <> 5 && $userType <> 1 && $userType <> 2)) {
                    if (($model->status < 3 && ($user->type <> 3 || $user->type <> 7)) || $model->status == 1 && $user->type == 4) {
...
//						$deadlinePayment = $this->dateThai($dateToPay->format("Y-m-d hh:mm:ss"), 1);
            if ($model->status < 3 && ($user->type != 4 || $user->type != 5 ) && Yii::app()->controller->action->id != "viewOrder") {
                if (($model->status > 3 && ($user->type == 2 || $user->type == 3 || $user->type == 7))) {
                    ?>
                ...
                if (($model->status == 3 && ($user->type == 3 || $user->type == 7)) || (($user->type == 5 || $user->type == 4) && $model->status >= 3)) {
                    ?>
                    <div class="table-condensed table-bordered" style="height: 31px;text-align: center; border-left: 1px; border-left-style: solid; border-left-color: #dddddd;"><p style="margin-top: 3px"><?php echo $model->orderNo ?></p></div>
                    <div class="table-condensed table-bordered" style="height: 40px;text-align: center; border-left: 1px; border-left-style: solid; border-left-color: #dddddd;"><p style="margin-top: 11px"><?php echo $model->invoiceNo == null ? '-' : $model->invoiceNo; ?></p></div>
                    <div class="table-condensed table-bordered" style="height: 40px;text-align: center; border-left: 1px; border-left-style: solid; border-left-color: #dddddd;"><p style="margin-top: 11px; color: red;"><?php echo $model->paymentDateTime == null ? "กำหนดชำระเงิน : " . $deadlinePayment : "วันที่ : " . $this->dateThai($model->paymentDateTime, 1); ?></p></div>
    ...
    <table class="table table-striped table-bordered table-condensed">
        <thead style="font-size:small">
            <tr>
                <?php
                if (isset($user)) {
                    if ($model->status < 3 && ($user->type != 4 || $user->type != 3 || $user->type != 5 || $user->type != 7 )) {
                        ?>
                        ...
                        if ($user->type == 1) {
                            ...
                            if (($model->status == 3 || $model->status == 4) && ($user->type == 3 || $user->type == 7)) {
                                ?>
        ...
        <tbody style="font-size:small">
            <tr>
                <td style="text-align: center"><?php
                    if (isset($user)) {
                        if ($model->status >= 3 && ($user->type == 4 || $user->type == 3 || $user->type == 5 || $user->type == 7)) {
                            if ($model->status >= 3 && ($user->type == 3 || $user->type == 7)) {
