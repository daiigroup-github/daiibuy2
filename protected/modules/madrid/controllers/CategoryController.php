...
class CategoryController extends MasterMadridController {
    ...
    public function actionIndex($id, $id2 = null) {
        ...
        if (isset($id2)) {
            $subCategorysId = $id2;
            $category2ToProducts = Category2ToProduct::model()->findAll('category1Id=' . $id . ' AND status=1 AND category2Id IN (' . $subCategorysId . ')');
        } else {
            $category2ToProducts = Category2ToProduct::model()->findAll('category1Id=' . $id . ' AND status=1 ');
        }
