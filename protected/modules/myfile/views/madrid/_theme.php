...
//                                                'style' => 'width:200px')) : "";
								...
								<?php if ($this->action->id == "view"): ?>
									<td style="color:red; width: 5%" id="total<?php echo $i; ?>">0.00<?php // echo number_format($item->product->calProductPromotionPrice(null, null) * $item->quantity, 0)                                                                                                                                                                  ?></td>
								<?php endif; ?>
							</tr>
							<?php
						else:
...
<script>
	...
	function select(sel, i)
	{
		var row = i;
		var attrName = sel.attributes['name'].value;
		var orderItemId = attrName.substr(11, 3);
//		alert(orderItemId);
...
//                alert(quantity);
				$("#quantityText_" + row).val(quantity);
				$("#productPrice" + orderItemId).html(data[productId]["price"]);
				$("#total" + row).html(moneyFormat(data[productId]["price"] * quantity));
				$("#productPic" + orderItemId).html(data[productId]["productImage"]);
				$("#priceHidden_" + row).val(data[productId]["price"]);
