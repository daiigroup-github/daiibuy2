...
class Swift_Mime_SimpleMessage extends Swift_Mime_MimePart implements Swift_Mime_Message
{
	 ...
	 */
	public function __construct(Swift_Mime_HeaderSet $headers, Swift_Mime_ContentEncoder $encoder, Swift_KeyCache $cache, $charset = null)
	{
	...
	public function setPriority($priority)
	{
		...
		if(!$this->_setHeaderFieldModel('X-Priority', sprintf('%d (%s)', $priority, $priorityMap[$priority])))
		{
	...
	{
		$this->setChildren(array_merge($this->getChildren(), array(
			$entity)));
		return $this;
	}
	...
	public function toString()
	{
		...
		{
			$this->setChildren(array_merge(array(
				$this->_becomeMimePart()), $children));
	...
	{
		$part = new parent($this->getHeaders()->newInstance(), $this->getEncoder(), $this->_getCache(), $this->_userCharset
		);
