...
class Swift_Mime_SimpleMimeEntity implements Swift_Mime_MimeEntity
{
	 ...
	 */
	public function __construct(Swift_Mime_HeaderSet $headers, Swift_Mime_ContentEncoder $encoder, Swift_KeyCache $cache)
	{
	...
	{
		if(!preg_match(
				'/^[a-z0-9\'\(\)\+_\-,\.\/:=\?\ ]{0,69}[a-z0-9\'\(\)\+_\-,\.\/:=\?]$/Di', $boundary))
		{
	...
	{
		return new self($this->_headers->newInstance(), $this->_encoder, $this->_cache);
	}
