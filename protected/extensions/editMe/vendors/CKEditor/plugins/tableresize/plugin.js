 ...
 */
		...
		(function() {
			...
			;
			function f(k) {
				var l = [], m = -1, n = k.getComputedStyle('direction') == 'rtl', o = e(k), p = new CKEDITOR.dom.element(k.$.tBodies[0]), q = p.getDocumentPosition();
				for (var r = 0, s = o.cells.length; r < s; r++) {
					...
					x = Math.max(w - v, 3);
					l.push({table: k, index: m, x: v, y: q.y, width: x, height: p.$.offsetHeight, rtl: n});
				}
				return l;
			}
			;
			...
			function i(k) {
				...
				;
				function w() {
					var D = l.index, E = CKEDITOR.tools.buildTableMap(l.table), F = [], G = [], H = Number.MAX_VALUE, I = H, J = l.rtl;
				...
				this.attachTo = function(D) {
					...
					l = D;
					n.setStyles({width: a(D.width), height: a(D.height), left: a(D.x), top: a(D.y)});
					b && n.setOpacity(0.25);
			...
			;
			CKEDITOR.plugins.add('tableresize', {requires: ['tabletools'], init: function(k) {
