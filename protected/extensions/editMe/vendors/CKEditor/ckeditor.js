		...
		(function() {
			...
			if (!window.CKEDITOR)
				window.CKEDITOR = (function() {
					var a = {timestamp: 'C6HH5UF', version: '3.6.4', revision: '7575', rnd: Math.floor(Math.random() * 900) + 100, _: {}, status: 'unloaded', basePath: (function() {
			...
			e.extend(d.node.prototype, {appendTo: function(h, i) {
					...
					while (k && k != j) {
						var l = k.parentNode;
						if (l)
							i.unshift(this.getIndex.call({$: k}, h));
						k = l;
					}
					return i;
					...
					while (i = i.previousSibling) {
						if (h && i.nodeType == 3 && (!i.nodeValue.length || i.previousSibling && i.previousSibling.nodeType == 3))
							continue;
						j++;
					}
					return j;
					...
					if (!i)
						j = j.parentNode;
					while (j) {
						if (j.nodeName && (k = j.nodeName.toLowerCase(), typeof h == 'string' ? k == h : k in h))
							return new d.node(j);
						j = j.parentNode;
					}
					return null;
					...
					if (this.type != 1)
						h = this.getParent();
					if (h && typeof h.$.isContentEditable != 'undefined')
						return!(h.$.isContentEditable || h.data('cke-editable'));
					else {
						var i = h;
						while (i) {
							if (i.is('body') || !!i.data('cke-editable'))
								break;
							if (i.getAttribute('contentEditable') == 'false')
								return true;
							else if (i.getAttribute('contentEditable') == 'true')
								break;
							i = i.getParent();
						}
						return false;
			...
			e.extend(h.prototype, {type: 1, addClass: function(i) {
					...
					var l = this;
					var j = l.getName();
					if (l.isReadOnly() || l.getComputedStyle('display') == 'none' || l.getComputedStyle('visibility') == 'hidden' || l.is('a') && l.data('cke-saved-name') && !l.getChildCount() || f.$nonEditable[j] || f.$empty[j])
						return false;
					if (i !== false) {
						var k = f[j] || f.span;
						return k && k['#'];
					...
					if (c && (b.ie7Compat || b.ie6Compat))
						return function(j, k) {
							...
							if (j == 'class')
								...
								l.$.checked = k;
							else if (j == 'contenteditable')
								i.call(l, 'contentEditable', k);
							else
								i.apply(l, arguments);
							return l;
						};
					else if (b.ie8Compat && b.secure)
						return function(j, k) {
							if (j == 'src' && k.match(/^http:\/\//))
								try {
					...
					var j = this.$.style;
					j.removeProperty ? j.removeProperty(i) : j.removeAttribute(e.cssStyleToDomStyle(i));
					...
					if (i instanceof d.window)
						i = m ? l.getBody() : l.getDocumentElement();
					function n(z, A) {
						if (/body|html/.test(i.getName()))
							i.getWindow().$.scrollBy(z, A);
						else {
							i.$.scrollLeft += z;
							i.$.scrollTop += A;
						}
					...
					if (x.y < 0 || y.y > 0)
						n(0, j === true ? x.y : j === false ? y.y : x.y < 0 ? x.y : y.y);
					if (k && (x.x < 0 || y.x > 0))
						n(x.x < 0 ? x.x : y.x, 0);
				}, setState: function(i) {
					var j = this;
					...
					j.load(z.split(','), function(D) {
						...
						for (var H in D) {
							...
							if (J) {
								...
								if (!I.langEntries || !I.langEntries[L])
						...
						a.scriptLoader.load(G, function() {
							...
							for (var N = 0; N < M.length; N++)
								for (var O = 0; O < E.length; O++) {
									var P = E[O];
									if (N === 0 && F[O] && P.lang)
										e.extend(x.lang, P.langEntries[F[O]]);
									if (P[M[N]])
										P[M[N]](x);
								}
				...
				a.htmlParser.fragment.fromHtml = function(q, r, s) {
					...
					function C(D, E, F) {
						...
						if (r && (!E.type || E.name == 'body')) {
							var H, I;
							if (D.attributes && (I = D.attributes['data-cke-real-element-type']))
								H = I;
							else
								H = D.name;
						...
						if (D._.isBlockLike && D.name != 'pre' && D.name != 'textarea') {
							...
							if (K && K.type == 3)
								if (!(L = e.rtrim(K.value)))
									D.children.length = J - 1;
								else
									K.value = L;
						}
						E.add(D);
					...
					t.onTagOpen = function(D, E, F, G) {
						...
						while (1) {
							...
							if (!H.isUnknown && !x.isUnknown && !J[D]) {
								if (x.isOptionalClose)
									t.onTagClose(I);
								else if (D in n && I in n) {
									var K = x.children, L = K[K.length - 1];
									if (!(L && L.name == 'li'))
										C(L = new a.htmlParser.element('li'), x);
									!H.returnPoint && (H.returnPoint = x);
									x = L;
								} else if (D in f.$listItem && I != D)
									t.onTagOpen(D == 'li' ? 'ul' : 'dl', {}, 0, 1);
								else if (I in l && I != D) {
									!H.returnPoint && (H.returnPoint = x);
									x = x.parent;
								} else {
					...
					t.onText = function(D) {
						...
						B();
						A();
						if (r && (!x.type || x.name == 'body') && e.trim(D))
							this.onTagOpen(r, {}, 0, 1);
						if (!z && !y)
							D = D.replace(/[\t\r\n ]{2,}|[\t\r\n]/g, ' ');
			...
			a.htmlParser.cssStyle = function() {
				...
				return{rules: n, populate: function(o) {
						var p = this.toString();
						if (p)
							o instanceof h ? o.setAttribute('style', p) : o instanceof a.htmlParser.element ? o.attributes.style = p : o.style = p;
					}, 'toString': function() {
						var o = [];
						for (var p in n)
							n[p] && o.push(p, ':', n[p], ';');
				...
				a.htmlParser.element.prototype = {type: 1, add: a.htmlParser.fragment.prototype.add, clone: function() {
						...
						if (n) {
							for (; ; ) {
								...
								q = p.name;
								if (!q) {
									for (var v = 0, w = this.children.length; v < w; v++)
										this.children[v].parent = p.parent;
						...
						if (s) {
							...
							if (v.type == 1)
								if (z > 0)
									...
									v = A(v, true) === false ? null : v.getPreviousSourceNode(true, C, A);
						} else {
							v = w;
							if (v.type == 1)
								if (!(v = v.getChild(y)))
									v = A(w, true) === false ? null : w.getNextSourceNode(true, C, A);
						}
						if (v && A(v) === false)
							v = null;
					}
			...
			(function() {
				var l = function(u) {
					...
					for (var O = G; O < E.length; O++) {
						...
						M = K.getNext();
						while (M) {
							if (M.equals(F[O]) || M.equals(z))
								break;
							N = M.getNext();
							if (v == 2)
								J.append(M.clone(true));
							else {
								M.remove();
								if (v == 1)
									J.append(M);
					...
					for (var P = G; P < F.length; P++) {
						...
						if (!E[P] || K.$.parentNode != E[P].$.parentNode) {
							M = K.getPrevious();
							while (M) {
								if (M.equals(E[P]) || M.equals(y))
									break;
								N = M.getPrevious();
								if (v == 2)
									J.$.insertBefore(M.$.cloneNode(true), J.$.firstChild);
								else {
									M.remove();
									if (v == 1)
										J.$.insertBefore(M.$, J.$.firstChild);
								}
								M = N;
							}
						}
					...
					if (v == 2) {
						...
						if (H && I && (y.$.parentNode != H.$.parentNode || z.$.parentNode != I.$.parentNode)) {
							var S = I.getIndex();
							if (C && I.$.parentNode == y.$.parentNode)
								S--;
							if (x && H.type == 1) {
				...
				function o(u) {
					var v = false, w = d.walker.bookmark(true), x = /^[\t\r\n ]*(?:&nbsp;|\xa0)$/;
					return function(y) {
						if (w(y))
							return true;
						if (y.type == 3) {
							if (c && x.test(y.getText()) && !v && !(u && y.getNext()))
								v = true;
							else if (y.hasAscendant('pre') || e.trim(y.getText()).length)
								return false;
						} else if (y.type == 1)
							if (!n[y.getName()])
								if (!c && y.is('br') && !v && !(u && y.getNext()))
									v = true;
								else
									return false;
						return true;
				...
				d.range.prototype = {clone: function() {
						...
						if (u) {
							...
							v.setAttribute('id', x + (z ? 'C' : 'S'));
						}
						if (!z) {
							w = v.clone();
							w.setHtml('&nbsp;');
							if (u)
								w.setAttribute('id', x + 'E');
							y = A.clone();
							y.collapse();
							y.insertNode(w);
						}
						...
						if (w) {
							...
							A.moveToPosition(v, 4);
						return{startNode: u ? x + (z ? 'C' : 'S') : v, endNode: u ? x + 'E' : w, serializable: u, collapsed: z};
						...
						if (v.type == 1) {
							y = v.getChildCount();
							if (y > x)
								v = v.getChild(x).getPreviousSourceNode(true);
							else if (y < 1)
								v = v.getPreviousSourceNode();
							else {
								v = v.$;
								while (v.lastChild)
									v = v.lastChild;
						...
						if (w.equals(x)) {
							if (u && w.type == 1 && z.startOffset == z.endOffset - 1)
								y = w.getChild(z.startOffset);
							else
								y = w;
						} else
							y = w.getCommonAncestor(x);
						...
						if (u.is && u.is('span') && u.data('cke-bookmark'))
							w.setStartAt(u, 3);
						if (v && v.is && v.is('span') && v.data('cke-bookmark'))
							w.setEndAt(v, 4);
					}, trim: function(u, v) {
						var C = this;
						...
						if ((!u || y) && w && w.type == 3) {
							if (!x) {
								...
								x = w.getIndex() + 1;
								w = w.getParent();
								if (C.startContainer.equals(C.endContainer))
									C.setEnd(z, C.endOffset - C.startOffset);
								else if (w.equals(C.endContainer))
									C.endOffset += 1;
							}
							C.setStart(w, x);
							if (y) {
									...
									while (B) {
										...
										if (B.type == 8) {
											...
											if (/[^\s\ufeff]/.test(F))
												B = null;
											E = /[\s\ufeff]$/.test(F);
										} else if ((B.$.offsetWidth > 0 || v && B.is('br')) && !B.data('cke-bookmark'))
											if (D && f.$removeEmpty[B.getName()]) {
												F = B.getText();
												if (/[^\s\ufeff]/.test(F))
													B = null;
												else {
								...
								while (A || B) {
									...
									while (B) {
										...
										if (B.type == 3) {
											...
											if ((B.$.offsetWidth > 0 || v && B.is('br')) && !B.data('cke-bookmark'))
												if (D && f.$removeEmpty[B.getName()]) {
													F = B.getText();
													if (/[^\s\ufeff]/.test(F))
														B = null;
						...
						if (x)
							...
							v.append(u);
						if (u.getParent().equals(y.endContainer))
							y.endOffset++;
						y.setStartBefore(u);
						...
						var w = this;
						if (u.type == 1 && f.$empty[u.getName()])
							v = u.getIndex(), u = u.getParent();
						w.startContainer = u;
						w.startOffset = v;
						if (!w.endContainer) {
							w.endContainer = u;
						...
						switch (v) {
							...
							case 2:
								if (u.type == 3)
									w.setStart(u, u.getLength());
								else
									w.setStart(u, u.getChildCount());
								break;
							case 3:
								w.setStartBefore(u);
								break;
						...
						switch (v) {
							...
							case 2:
								if (u.type == 3)
									w.setEnd(u, u.getLength());
								else
									w.setEnd(u, u.getChildCount());
								break;
							case 3:
								w.setEndBefore(u);
								break;
						...
						function u(v, w) {
							while (v) {
								if (v.type == 1)
									if (v.getAttribute('contentEditable') == 'false' && !v.data('cke-editable'))
										return 0;
									else if (v.is('html') || v.getAttribute('contentEditable') == 'true' && (v.contains(w) || v.equals(w)))
										break;
								v = v.getParent();
							}
							return 1;
						...
						function x(z, A) {
							var B;
							if (z.type == 1 && z.isEditable(false))
								B = z[v ? 'getLast' : 'getFirst'](t);
							if (!A && !B)
								B = z[v ? 'getPrevious' : 'getNext'](t);
							return B;
						}
						;
						if (u.type == 1 && !u.isEditable(false)) {
						...
						while (u) {
							if (u.type == 3) {
								if (v && this.checkEndOfBlock() && w.test(u.getText()))
									this.moveToPosition(u, 3);
								else
									this.moveToPosition(u, v ? 4 : 3);
								y = 1;
								break;
							...
							if (u.type == 1)
								if (u.isEditable()) {
									...
									y = 1;
								} else if (v && u.is('br') && this.checkEndOfBlock())
									this.moveToPosition(u, 3);
							u = x(u, y);
						}
						return!!y;
						...
						var u = this.clone();
						u.optimize();
						if (u.startContainer.type != 1 || u.endContainer.type != 1)
							return null;
			...
			(function() {
				...
				j.add(m, {requires: ['dialog'], availableLangs: {cs: 1, cy: 1, da: 1, de: 1, el: 1, en: 1, eo: 1, fa: 1, fi: 1, fr: 1, gu: 1, he: 1, it: 1, mk: 1, nb: 1, nl: 1, no: 1, 'pt-br': 1, ro: 1, tr: 1, ug: 1, vi: 1, 'zh-cn': 1}, init: function(o) {
						var p = this;
			...
			(function() {
				...
				function t(B, C, D, E) {
					...
					if (G) {
						B.removeAttribute('dir');
						if (C != B.getComputedStyle('direction'))
							B.setAttribute('dir', C);
					} else
						B.setAttribute('dir', C);
				...
				function u(B, C, D) {
					...
					if (B.checkBoundaryOfElement(E, 1) && B.checkBoundaryOfElement(E, 2)) {
						var F;
						while (E && E.type == 1 && (F = E.getParent()) && F.getChildCount() == 1 && !(E.getName() in C))
							E = F;
					...
					return function(C) {
						...
						if (F && F.length) {
							...
							while (J = I.getNextRange(1)) {
								...
								if (!L || L && !(L.type == 1 && L.getName() in n))
									L = u(J, m, E);
								L && t(L, B, C, G);
								...
								M = J.createIterator();
								M.enlargeBr = E != 2;
								while (N = M.getNextParagraph(E == 1?'p':'div'))
									t(N, B, C, G);
							}
							h.clearAllMarkers(G);
			...
			(function() {
				k.button.prototype = {render: function(m, n) {
						...
						var t = {id: p, button: this, editor: m, focus: function() {
							...
							var A;
							if (t.onfocus)
								A = t.onfocus(t, new d.event(z)) !== false;
						...
						if (this.modes) {
							...
							m.on('beforeModeUnload', function() {
								if (m.mode && this._.state != 0)
									w[m.mode] = this._.state;
							}, this);
							m.on('mode', x, this);
						...
						if (this.hasArrow)
							n.push('<span class="cke_buttonarrow">' + (b.hc ? '&#9660;' : '&nbsp;') + '</span>');
						n.push('</a>', '</span>');
						if (this.onRender)
							this.onRender();
						return t;
			...
			(function() {
				...
				o.prototype = {exec: function(y, z) {
						...
						var A = n(y, this.type);
						if (!A)
							alert(y.lang.clipboard[this.type + 'Error']);
						return A;
					}};
				var p = {canUndo: false, exec: c ? function(y) {
						...
						try {
							if (!y.document.getBody().fire('beforepaste') && !y.document.$.execCommand('Paste', false, null))
								throw 0;
						} catch (z) {
							setTimeout(function() {
								y.fire('pasteDialog');
				...
				j.add('clipboard', {requires: ['dialog', 'htmldataprocessor'], init: function(y) {
						...
						function z(A, B, C, D) {
							...
							if (y.addMenuItems)
								y.addMenuItem(B, {label: E, command: B, group: 'clipboard', order: D});
						}
						;
						...
						y.on('contentDom', function() {
							...
							A.on(!c ? 'paste' : 'beforepaste', function(B) {
								...
								s.call(y, B, D.mode, function(E) {
									if (!(E = e.trim(E.replace(/<span[^>]+data-cke-bookmark[^<]*?<\/span>/ig, ''))))
										return;
									var F = {};
									F[D.mode] = E;
									y.fire('paste', F);
						...
						if (y.contextMenu)
							y.contextMenu.addListener(function(A, B) {
			...
			j.add('colorbutton', {requires: ['panelbutton', 'floatpanel', 'styles'], init: function(m) {
					...
					function r(t, u, v) {
						...
						if (n.colorButton_enableMore === undefined || n.colorButton_enableMore)
							w.push('</tr><tr><td colspan=8 align=center><a class="cke_colormore" _cke_focus=1 hidefocus=true title="', o.more, '" onclick="CKEDITOR.tools.callFunction(', y, ",'?','", u, "');return false;\" href=\"javascript:void('", o.more, "')\"", ' role="option">', o.more, '</a></td>');
						w.push('</tr></table>');
						return w.join('');
					}
					;
			...
			(function() {
				...
				j.add('dialogadvtab', {createAdvancedTab: function(o, p) {
						...
						if (p.styles || p.classes) {
							s = [];
							if (p.styles)
								s.push({id: 'advStyles', att: 'style', type: 'text', label: q.styles, 'default': '', validate: a.dialog.validate.inlineStyle(q.invalidInlineStyle), onChange: function() {
				...
				j.add('elementspath', {requires: ['selection'], init: function(o) {
						...
						o.on('themeSpace', function(x) {
							if (x.data.space == 'bottom')
								x.data.html += '<span id="' + p + '_label" class="cke_voice_label">' + o.lang.elementsPath.eleLabel + '</span>' + '<div id="' + p + '" class="cke_path" role="group" aria-labelledby="' + p + '_label">' + n + '</div>';
						});
						function t(x) {
							o.focus();
							...
							if (y.is('body')) {
								...
								o.getSelection().selectElement(y);
						}
						;
						var u = e.addFunction(t), v = e.addFunction(function(x, y) {
							...
							switch (y.getKeystroke()) {
								...
								case 2228224 + 9:
									A = a.document.getById(z + (x - 1));
									if (!A)
										A = a.document.getById(z + (o._.elementsPath.list.length - 1));
									A.focus();
									return false;
								case 27:
									o.focus();
						...
						o.on('selectionChange', function(x) {
							...
							while (A) {
								var F = 0, G;
								if (A.data('cke-display-name'))
									G = A.data('cke-display-name');
								else if (A.data('cke-real-element-type'))
									G = A.data('cke-real-element-type');
								else
									G = A.getName();
								...
								if (!F) {
									var J = D.push(A) - 1, K = '';
									if (y.opera || y.gecko && y.mac)
										K += ' onkeypress="return false;"';
									if (y.gecko)
										K += ' onblur="this.style.cssText = this.style.cssText;"';
				...
				j.enterkey = {enterBlock: function(t, u, v, w) {
						...
						if (y && z) {
							...
							if (B && B.getParent().is('blockquote')) {
								...
								if (!B.getPrevious().getFirst(d.walker.invisible(1)))
									B.getPrevious().remove();
								if (!B.getNext().getFirst(d.walker.invisible(1)))
									B.getNext().remove();
						...
						if (!G && !H) {
							if (F.is('li') && (I = F.getFirst(d.walker.invisible(true))) && I.is && I.is('ul', 'ol'))
								(c ? x.createText('\xa0') : x.createElement('br')).insertBefore(I);
							if (F)
								v.moveToElementEditStart(F);
						} else {
							var J, K;
							if (E) {
							...
							if (L)
								for (var M = 0, N = L.elements.length; M < N; M++) {
									var O = L.elements[M];
									if (O.equals(L.block) || O.equals(L.blockLimit))
										break;
			...
			(function() {
				...
				function r(v, w, x, y) {
					...
					for (var B in y) {
						z = y[B];
						if (z.type == 'hbox' || z.type == 'vbox' || z.type == 'fieldset')
							r(v, w, x, z.children);
						if (!z.filebrowser)
							continue;
			...
			(function() {
				...
				j.add('flash', {init: function(p) {
						...
						if (p.addMenuItems)
							p.addMenuItems({flash: {label: p.lang.flash.properties, command: 'flash', group: 'flash'}});
						p.on('doubleclick', function(q) {
							var r = q.data.element;
							if (r.is('img') && r.data('cke-real-element-type') == 'flash')
								q.data.dialog = 'flash';
						});
						if (p.contextMenu)
							p.contextMenu.addListener(function(q, r) {
								if (q && q.is('img') && !q.isReadOnly() && q.data('cke-real-element-type') == 'flash')
									return{flash: 2};
							});
					}, afterInit: function(p) {
						var q = p.dataProcessor, r = q && q.dataFilter;
						if (r)
							r.addRules({elements: {'cke:object': function(s) {
			...
			if (c)
				h.prototype.hasAttribute = e.override(h.prototype.hasAttribute, function(m) {
					return function(n) {
						var q = this;
			...
			(function() {
				...
				function o(T) {
					...
					while (V && V.type == 3 && !e.trim(V.value))
						V = T.children[--U];
					return V;
				}
				;
				function p(T, U) {
					...
					if (W) {
						if ((U || !c) && W.type == 1 && W.name == 'br')
							V.pop();
						if (W.type == 3 && m.test(W.value))
							V.pop();
					}
				}
				;
				function q(T, U, V) {
					if (!U && (!V || typeof V == 'function' && V(T) === false))
						return false;
					if (U && c && (document.documentMode > 7 || T.name in f.tr || T.name in f.$listItem))
						return false;
					var W = o(T);
				...
				function r(T, U) {
					return function(V) {
						...
						if (q(V, !T, U))
							if (T || c)
								V.add(new a.htmlParser.text('\xa0'));
							else
								V.add(new a.htmlParser.element('br', {}));
					};
				}
				;
				...
				delete u.pre;
				var w = {elements: {}, attributeNames: [[/^on/, 'data-cke-pa-on']]}, x = {elements: {}};
				for (v in u)
					x.elements[v] = r();
				var y = {elementNames: [[/^cke:/, ''], [/^\?xml:namespace$/, '']], attributeNames: [[/^data-cke-(saved|pa)-/, ''], [/^data-cke-.*/, ''], ['hidefocus', '']], elements: {$: function(T) {
							var U = T.attributes;
							if (U) {
								if (U['data-cke-temp'])
									return false;
							...
							if (!(T.children.length || T.attributes.name || T.attributes['data-cke-saved-name']))
								return false;
						}, span: function(T) {
							if (T.attributes['class'] == 'Apple-style-span')
								delete T.name;
						}, pre: function(T) {
							c && p(T);
						}, html: function(T) {
				...
				if (c)
					y.attributes.style = function(T, U) {
				...
				function z(T) {
					...
					if (U.contenteditable != 'false')
						U['data-cke-editable'] = U.contenteditable ? 'true' : 1;
				...
				function I(T) {
					return T.replace(B, function(U, V, W) {
						return '<' + V + W.replace(C, function(X, Y) {
							if (!/^on/.test(Y) && W.indexOf('data-cke-saved-' + Y) == -1)
								return ' data-cke-saved-' + X + ' data-cke-' + a.rnd + '-' + X;
							return X;
						}) + '>';
					});
				}
				...
				j.add('htmldataprocessor', {requires: ['htmlwriter'], init: function(T) {
						...
						var V = {elements: {}};
						for (v in u)
							V.elements[v] = r(true, T.config.fillEmptyBlocks);
			...
			(function() {
				j.add('iframe', {requires: ['dialog', 'fakeobjects'], init: function(m) {
						...
						m.on('doubleclick', function(p) {
							...
							if (q.is('img') && q.data('cke-real-element-type') == 'iframe')
								p.data.dialog = 'iframe';
						});
						if (m.addMenuItems)
							m.addMenuItems({iframe: {label: o.title, command: 'iframe', group: 'image'}});
						if (m.contextMenu)
							m.contextMenu.addListener(function(p, q) {
								if (p && p.is('img') && p.data('cke-real-element-type') == 'iframe')
									return{iframe: 2};
							});
					}, afterInit: function(m) {
						var n = m.dataProcessor, o = n && n.dataFilter;
						if (o)
							o.addRules({elements: {iframe: function(p) {
			...
			(function() {
				j.add('image', {requires: ['dialog'], init: function(o) {
						...
						o.on('doubleclick', function(q) {
							var r = q.data.element;
							if (r.is('img') && !r.data('cke-realelement') && !r.isReadOnly())
								q.data.dialog = 'image';
						});
						if (o.addMenuItems)
							o.addMenuItems({image: {label: o.lang.image.menu, command: 'image', group: 'image'}});
						if (o.contextMenu)
							o.contextMenu.addListener(function(q, r) {
								if (m(o, q))
									return{image: 2};
							});
				...
				function m(o, p) {
					...
					if (p && p.is('img') && !p.data('cke-realelement') && !p.isReadOnly())
						return p;
				}
				;
				function n(o) {
			...
			(function() {
				...
				function p(t) {
					...
					if (w)
						return B.setState(2);
					if (!B.useIndentClasses && B.name == 'indent')
						return B.setState(2);
					if (!x)
						return B.setState(0);
					if (B.useIndentClasses) {
						...
						if (B.name == 'outdent' && !z || B.name == 'indent' && z == u.config.indentClasses.length)
							return B.setState(0);
						return B.setState(2);
					} else {
			...
			(function() {
				function m(q, r) {
					...
					var s;
					if (r)
						s = q.getComputedStyle('text-align');
					else {
			...
			j.add('link', {requires: ['fakeobjects', 'dialog'], init: function(m) {
					...
					m.on('selectionChange', function(p) {
						...
						if (r && r.getName() == 'a' && r.getAttribute('href') && r.getChildCount())
							q.setState(2);
						else
							q.setState(0);
					});
					m.on('doubleclick', function(p) {
						...
						if (!q.isReadOnly())
							...
							} else if (j.link.tryRestoreFakeAnchor(m, q))
								p.data.dialog = 'anchor';
					});
					if (m.addMenuItems)
						m.addMenuItems({anchor: {label: m.lang.anchor.menu, command: 'anchor', group: 'anchor', order: 1}, removeAnchor: {label: m.lang.anchor.remove, command: 'removeAnchor', group: 'anchor', order: 5}, link: {label: m.lang.link.menu, command: 'link', group: 'link', order: 1}, unlink: {label: m.lang.unlink, command: 'unlink', group: 'link', order: 5}});
					if (m.contextMenu)
						m.contextMenu.addListener(function(p, q) {
							if (!p || p.isReadOnly())
								return null;
							var r = j.link.tryRestoreFakeAnchor(m, p);
							if (!r && !(r = j.link.getSelectedLink(m)))
								return null;
							var s = {};
							if (r.getAttribute('href') && r.getChildCount())
								s = {link: 2, unlink: 2};
							if (r && r.hasAttribute('name'))
								s.anchor = s.removeAnchor = 2;
							return s;
						});
				}, afterInit: function(m) {
					var n = m.dataProcessor, o = n && n.dataFilter, p = n && n.htmlFilter, q = m._.elementsPath && m._.elementsPath.filters;
					if (o)
						o.addRules({elements: {a: function(r) {
									var s = r.attributes;
									if (!s.name)
										return null;
									...
									if (j.link.synAnchorSelector) {
										...
										if (s.name && (!v || v.indexOf(u) < 0))
											s['class'] = (v || '') + ' ' + u;
										if (t && j.link.emptyAnchorFix) {
											s.contenteditable = 'false';
											s['data-cke-editable'] = 1;
										}
									} else if (j.link.fakeAnchor && t)
										return m.createFakeParserElement(r, 'cke_anchor', 'anchor');
									return null;
								}}});
					if (j.link.emptyAnchorFix && p)
						p.addRules({elements: {a: function(r) {
									delete r.attributes.contenteditable;
								}}});
					if (q)
						q.push(function(r, s) {
							if (s == 'a')
								if (j.link.tryRestoreFakeAnchor(m, r) || r.getAttribute('name') && (!r.getAttribute('href') || !r.getChildCount()))
									return 'anchor';
						});
				}});
			...
			a.removeAnchorCommand.prototype = {exec: function(m) {
					...
					if (n && (p = n.getSelectedElement()) && (j.link.fakeAnchor && !p.getChildCount() ? j.link.tryRestoreFakeAnchor(m, p) : p.is('a')))
						p.remove(1);
					else if (p = j.link.getSelectedLink(m))
			...
			(function() {
				...
				j.list = {listToArray: function(N, O, P, Q, R) {
						...
						for (var S = 0, T = N.getChildCount(); S < T; S++) {
							var U = N.getChild(S);
							if (U.type == 1 && U.getName() in f.$list)
								j.list.listToArray(U, O, P, Q + 1);
							if (U.$.nodeName.toLowerCase() != 'li')
								continue;
							var V = {parent: N, indent: Q, element: U, contents: []};
							if (!R) {
								V.grandparent = N.getParent();
								if (V.grandparent && V.grandparent.$.nodeName.toLowerCase() == 'li')
									V.grandparent = V.grandparent.getParent();
							} else
								V.grandparent = R;
							if (O)
								h.setMarker(O, U, 'listarray_index', P.length);
							P.push(V);
							for (var W = 0, X = U.getChildCount(), Y; W < X; W++) {
								Y = U.getChild(W);
								if (Y.type == 1 && m[Y.getName()])
									j.list.listToArray(Y, O, P, Q + 1, V.grandparent);
								else
									V.contents.push(Y);
							}
						}
						...
						while (1) {
							...
							if (ac.indent == X) {
								...
								if (Z != V.getDirection(1))
									Y.setAttribute('dir', Z);
								for (S = 0; S < ac.contents.length; S++)
									Y.append(ac.contents[S].clone(1, 1));
								W++;
							} else if (ac.indent == Math.max(X, 0) + 1) {
								var ae = N[W - 1].element.getDirection(1), af = j.list.arrayToList(N, null, W, Q, ae != Z ? Z : null);
								if (!Y.getChildCount() && c && !(T.$.documentMode > 7))
									Y.append(T.createText('\xa0'));
								Y.append(af.listNode);
								W = af.nextIndex;
							} else if (ac.indent == -1 && !P && ad) {
								if (m[ad.getName()]) {
									Y = ac.element.clone(false, true);
									if (Z != ad.getDirection(1))
										Y.setAttribute('dir', Z);
								} else
									Y = new d.documentFragment(T);
								...
								for (S = 0; S < am; S++) {
									...
									if (al.type == 1 && al.isBlockBoundary()) {
										if (ag && !al.getDirection())
											al.setAttribute('dir', Z);
										t(ah, al);
										ai && al.addClass(ai);
									} else if (ak) {
										if (!aa) {
								...
								if (Y.type == 11 && W != N.length - 1) {
									var an = Y.getLast();
									if (an && an.type == 1 && an.getAttribute('type') == '_moz')
										an.remove();
									if (!(an = Y.getLast(q) && an.type == 1 && an.getName() in f.$block))
										Y.append(T.createElement('br'));
								}
								...
								return null;
							aa = null;
							if (N.length <= W || Math.max(N[W].indent, 0) < X)
								break;
						}
						if (O) {
				...
				function v(N, O, P, Q) {
					...
					for (var T = 0; T < O.contents.length; T++) {
						...
						if (!U || U.getCustomData('list_item_processed'))
							continue;
						S.push(U);
				...
				function x(N, O, P) {
					...
					for (var V = 0; V < Q.length; V++)
						U = U.getCommonAncestor(Q[V].getParent());
					...
					while (S.length) {
						...
						if (ae.is('pre') || w.test(ae.getName()))
							ae.appendTo(af);
						else {
				...
				function y(N, O, P) {
					...
					function ac(ad) {
						if ((aa = Z[ad ? 'getFirst' : 'getLast']()) && !(aa.is && aa.isBlockBoundary()) && (ab = O.root[ad ? 'getPrevious' : 'getNext'](d.walker.whitespaces(true))) && !(ab.is && ab.isBlockBoundary({br: 1})))
							N.document.createElement('br')[ad ? 'insertBefore' : 'insertAfter'](aa);
					}
					;
					ac(true);
					ac();
				...
				function B(N, O, P, Q) {
					...
					while (R = N[Q ? 'getLast' : 'getFirst'](A)) {
						if ((S = R.getDirection(1)) !== O.getDirection(1))
				...
				z.prototype = {exec: function(N) {
						...
						while ((T = Y.getNextRange()) && ++Z) {
							...
							if (ab.type == 1 && ab.getName() == 'td')
								T.setStartAt(aa.startNode, 1);
							if (ac.type == 1 && ac.getName() == 'td')
								T.setEndAt(aa.endNode, 2);
							var ad = T.createIterator(), ae;
							ad.forceBrBreak = aq.state == 2;
							while (ae = ad.getNextParagraph()) {
								if (ae.getCustomData('list_block'))
									continue;
								else
									h.setMarker(X, ae, 'list_block', 1);
								...
								if (aj)
									continue;
								var ao = ak;
								if (ao.getCustomData('list_group_object_' + Z))
									ao.getCustomData('list_group_object_' + Z).contents.push(ae);
								else {
						...
						while (W.length > 0) {
							an = W.shift();
							if (aq.state == 2) {
								if (m[an.root.getName()])
									v.call(aq, N, an, X, ap);
								else
									x.call(aq, N, an, ap);
							} else if (aq.state == 1 && m[an.root.getName()])
								y.call(aq, N, an, X);
						}
				...
				function G(N) {
					return function(O) {
						...
						if (S && (S.name && S.name == 'br' || S.value && (T = S.value.match(E)))) {
							var U = S;
							if (!(T && T.index) && U == P[0])
								P[0] = N || c ? new a.htmlParser.text('\xa0') : new a.htmlParser.element('br', {});
							else if (U.name == 'br')
								P.splice(Q - 1, 1);
							else
								U.value = U.value.replace(E, '');
						}
					};
				}
				;
				var H = {elements: {}};
				for (var I in D.$listItem)
					H.elements[I] = G();
				var J = {elements: {}};
				for (I in D.$listItem)
					J.elements[I] = G(true);
				...
				function L(N, O, P) {
					...
					O.trim(false, true);
					var R = O.createBookmark(), S = new d.elementPath(O.startContainer), T = S.lastElement.getAscendant('li', 1), U = S.block.getBogus();
					U && U.remove();
					var V = Q.getLast();
					if (V && V.type == 1 && V.is('br'))
						V.remove();
					var W = O.startContainer.getChild(O.startOffset);
					if (W)
						Q.insertBefore(W);
					else
						O.startContainer.append(Q);
					var X = new d.elementPath(P.startContainer), Y = P.startContainer.getAscendant('li', 1);
					if (Y) {
						var Z = M(Y);
						if (Z)
							if (T.contains(Y)) {
					...
					while (P.checkStartOfBlock() && P.checkEndOfBlock()) {
						X = new d.elementPath(P.startContainer);
						var aa = X.block, ab;
						if (aa.is('li')) {
							ab = aa.getParent();
							if (aa.equals(ab.getLast(q)) && aa.equals(ab.getFirst(q)))
								aa = ab;
						}
						P.moveToPosition(aa, 3);
						aa.remove();
					}
					var ac = P.clone(), ad = N.document.getBody();
					ac.setEndAt(ad, 2);
					var ae = new d.walker(ac);
					ae.evaluator = function(ag) {
						return q(ag) && !r(ag);
					};
					var af = ae.next();
					if (af && af.type == 1 && af.getName() in f.$list)
						C(af);
					O.moveToBookmark(R);
					O.select();
					N.selectionChange(1);
				...
				j.add('list', {init: function(N) {
						...
						N.on('key', function(Q) {
							...
							if (N.mode == 'wysiwyg' && R in {8: 1, 46: 1}) {
								...
								if (!T.collapsed)
									return;
								var U = R == 8, V = N.document.getBody(), W = new d.walker(T.clone());
								W.evaluator = function(ah) {
									return q(ah) && !r(ah);
								};
								var X = T.clone();
								if (U) {
									var Y, Z, aa = new d.elementPath(T.startContainer);
									if ((Y = aa.contains(m)) && T.checkBoundaryOfElement(Y, 1) && (Y = Y.getParent()) && Y.is('li') && (Y = M(Y))) {
										Z = Y;
										Y = Y.getPrevious(q);
										X.moveToPosition(Y && r(Y) ? Y : Z, 3);
									} else {
										W.range.setStartAt(V, 1);
										W.range.setEnd(T.startContainer, T.startOffset);
										Y = W.previous();
										if (Y && Y.type == 1 && (Y.getName() in m || Y.is('li'))) {
											if (!Y.is('li')) {
												W.range.selectNodeContents(Y);
												W.reset();
												W.evaluator = K;
												Y = W.previous();
											}
											Z = Y;
											X.moveToElementEditEnd(Z);
										}
									}
									if (Z) {
										L(N, X, T);
									...
									var ab = T.startContainer.getAscendant('li', 1);
									if (ab) {
										W.range.setEndAt(V, 2);
										var ac = ab.getLast(q), ad = ac && K(ac) ? ac : ab, ae = 0, af = W.next();
										if (af && af.type == 1 && af.getName() in m && af.equals(ac)) {
											ae = 1;
											af = W.next();
										} else if (T.checkBoundaryOfElement(ad, 2))
											ae = 1;
										if (ae && af) {
											var ag = T.clone();
											ag.moveToElementEditStart(af);
											L(N, X, ag);
											Q.cancel();
										}
									}
			...
			j.add('pagebreak', {init: function(m) {
					...
					b.opera && m.on('contentDom', function() {
						m.document.on('click', function(o) {
							...
							if (p.is('div') && p.hasClass('cke_pagebreak'))
								m.getSelection().selectElement(p);
						});
					});
				}, afterInit: function(m) {
					var n = m.lang.pagebreakAlt, o = m.dataProcessor, p = o && o.dataFilter, q = o && o.htmlFilter;
					if (q)
						q.addRules({attributes: {'class': function(r, s) {
					...
					if (p)
						p.addRules({elements: {div: function(r) {
			...
			j.pagebreakCmd = {exec: function(m) {
					...
					for (var q, r = p.length - 1; r >= 0; r--) {
						...
						if (r == p.length - 1) {
							...
							if (!s || s.type == 1 && !s.isEditable())
								q.fixBlock(true, m.config.enterMode == 3 ? 'div' : 'p');
							q.select();
						}
					}
					m.fire('saveSnapshot');
						...
						n.on('paste', function(q) {
							...
							if ((s = r.html) && (o || /(class=\"?Mso|style=\"[^\"]*\bmso\-|w:WordDocument)/.test(s))) {
								var t = this.loadFilterRules(function() {
									if (t)
										n.fire('paste', r);
									else if (!n.config.pasteFromWordPromptCleanup || o || confirm(n.lang.pastefromword.confirmCleanup))
										r.html = a.cleanWord(s, n);
								});
								t && q.cancel();
							}
						}, this);
			...
			(function() {
				var m, n = {modes: {wysiwyg: 1, source: 1}, canUndo: false, readOnly: 1, exec: function(p) {
						...
						if (r.fullPage)
							q = p.getData().replace(/<head>/, '$&' + s).replace(/[^>]*(?=<\/title>)/, '$& &mdash; ' + p.lang.preview);
						else {
							var u = '<body ', v = p.document && p.document.getBody();
							if (v) {
								if (v.getAttribute('id'))
									u += 'id="' + v.getAttribute('id') + '" ';
								if (v.getAttribute('class'))
									u += 'class="' + v.getAttribute('class') + '" ';
							}
							u += '>';
			...
			j.add('resize', {init: function(m) {
					...
					if (n.resize_enabled !== false) {
						...
						function u(x) {
							...
							if (s)
								A = Math.max(n.resize_minWidth, Math.min(C, n.resize_maxWidth));
							if (t)
								B = Math.max(n.resize_minHeight, Math.min(D, n.resize_maxHeight));
							m.resize(s ? A : null, B);
						}
						;
						function v(x) {
						...
						m.on('themeSpace', function(x) {
							if (x.data.space == 'bottom') {
								var y = '';
								if (s && !t)
									y = ' cke_resizer_horizontal';
								if (!s && t)
									y = ' cke_resizer_vertical';
			...
			(function() {
				...
				j.scayt = {engineLoaded: false, instances: {}, controlInfo: {}, setControlInfo: function(t, u) {
						...
						v[3] = '1';
						for (var w = 0; w < 4; w++)
							u[w] = typeof window.scayt != 'undefined' && typeof window.scayt.uiTags != 'undefined' ? parseInt(v[w], 10) && window.scayt.uiTags[w] : parseInt(v[w], 10);
						return u;
				...
				j.add('scayt', {requires: ['menubutton'], beforeInit: function(t) {
						...
						if (z[0] == 1)
							B.scaytOptions = {label: C.options, group: A, onClick: function() {
									n = 'options';
									t.openDialog(m);
								}};
						if (z[1] == 1)
							B.scaytLangs = {label: C.langs, group: A, onClick: function() {
									n = 'langs';
									t.openDialog(m);
								}};
						if (z[2] == 1)
							B.scaytDict = {label: C.dictionariesTab, group: A, onClick: function() {
									n = 'dictionaries';
									t.openDialog(m);
								}};
						...
						if (t.contextMenu && t.addMenuItems)
							t.contextMenu.addListener(function(E, F) {
								if (!q.isScaytEnabled(t) || F.getRanges()[0].checkReadOnly())
									return null;
								...
								if (G.fireOnContextMenu)
									G.fireOnContextMenu(t);
								return x;
							});
						var D = function() {
							t.removeListener('showScaytState', D);
							if (!b.opera && !b.air)
								y.setState(q.isScaytEnabled(t) ? 1 : 2);
							else
								y.setState(0);
						};
			...
			j.add('sourcearea', {requires: ['editingblock'], init: function(m) {
					...
					m.on('readOnly', function() {
						if (m.mode == 'source')
							if (m.readOnly)
								m.textarea.setAttribute('readOnly', 'readonly');
							else
								m.textarea.removeAttribute('readOnly');
					});
			...
			(function() {
				j.add('stylescombo', {requires: ['richcombo', 'styles'], init: function(n) {
						...
						n.ui.addRichCombo('Styles', {label: p.label, title: p.panelTitle, className: 'cke_styles', panel: {css: n.skin.editor.css.concat(o.contentsCss), multiSelect: true, attributes: {'aria-label': p.panelTitle}}, init: function() {
								...
								if (!x[1])
									B.hideGroup(p['panelTitle' + String(1)]);
								if (!x[2])
									B.hideGroup(p['panelTitle' + String(2)]);
								if (!x[3])
									B.hideGroup(p['panelTitle' + String(3)]);
							}, reset: function() {
								if (s) {
									delete s._.panel;
									delete s._.list;
			...
			(function() {
				...
				function u(G) {
					...
					for (var P = 0, Q = L.length; P < Q; P++)
						for (var R = 0, S = L[P].length; R < S; R++) {
							if (L[P][R] == I.$)
								M = R;
							if (L[P][R] == J.$)
								N = R;
				...
				j.tabletools = {requires: ['table', 'dialog', 'contextmenu'], init: function(G) {
						var H = G.lang.table;
						...
						G.addCommand('tableDelete', {exec: function(I) {
								...
								if (!L)
									return;
								var M = L.getParent();
								if (M.getChildCount() == 1 && !M.is('body', 'td', 'th'))
									L = M;
			...
			(function() {
				...
				j.add('tab', {requires: ['keystrokes'], init: function(q) {
						...
						if (r)
							q.on('key', function(u) {
								if (u.data.keyCode == 9 && q.execCommand('selectNextCell') || u.data.keyCode == 2228224 + 9 && q.execCommand('selectPreviousCell'))
									u.cancel();
							});
						if (b.webkit || b.gecko)
							q.on('key', function(u) {
							...
							switch (s) {
								...
								case 38:
									...
									do {
										t = t.previous;
										if (!t && w)
											t = r.toolbar.items[r.toolbar.items.length - 1];
									} while (t && !t.focus);
									if (t)
										t.focus();
									else {
						...
						o.on('themeSpace', function(r) {
							if (r.data.space == o.config.toolbarLocation) {
								...
								if (o.config.toolbarCanCollapse) {
									...
									if (!u)
										t.push(' cke_toolbox_collapser_min');
						...
						o.on('destroy', function() {
							...
							for (; s < r.length; s++) {
								...
								for (t = 0; t < u.length; t++) {
									v = u[t];
									if (v.clickFn)
										e.removeFunction(v.clickFn);
									if (v.keyDownFn)
										e.removeFunction(v.keyDownFn);
								}
							}
						});
				...
				o.prototype = {type: function(s) {
						...
						if (C || B) {
							...
							e.setTimeout(function() {
								...
								if (c)
									F = F.replace(/\s+data-cke-expando=".*?"/g, '');
								if (D.contents != F && E == G.snapshots.length) {
									G.typing = true;
									if (!G.save(false, D, false))
										G.snapshots.splice(G.index + 1, G.snapshots.length - G.index - 1);
									G.hasUndo = true;
									G.hasRedo = false;
									G.typesCount = 1;
									G.modifiersCount = 1;
			...
			(function() {
				...
				function t(E) {
					...
					I = I.replace(/\n/g, '<br>');
					if (!(H || c))
						I = I.replace(new RegExp('<br>(?=</' + J + '>)'), function(O) {
							return e.repeat(O, 2);
						});
					if (b.gecko || b.webkit) {
						...
						for (var M = 0; M < K.elements.length; M++) {
							...
							if (N in f.$inline)
								L.unshift(K.elements[M].getOuterHtml().match(/^<.*?>/));
							else if (N in f.$block)
								break;
						}
						I = L.join('') + I;
					}
				...
				function u(E) {
					...
					for (var O = G.length - 1; O >= 0; O--) {
						...
						if (!K.checkReadOnly()) {
							...
							var P, Q;
							if (I)
								while ((P = K.getCommonAncestor(0, 1)) && (Q = f[P.getName()]) && !(Q && Q[H])) {
									if (P.getName() in f.span)
										K.splitElement(P);
				...
				function C(E) {
					...
					if (!J || J == 1 && I.type == 1 && I.hasAttribute('_moz_editor_bogus_node')) {
						...
						G.$.dispatchEvent(O);
						if (M != L.$.scrollTop || N != L.$.scrollLeft)
							K.getWindow().$.scrollTo(N, M);
						J && H.getFirst().remove();
				...
				function D(E) {
					...
					if (b.gecko) {
						...
						if (M && M.isBlockBoundary() && !(N && N.type == 1 && N.isBlockBoundary()) && !M.is('pre') && !M.getBogus())
							M.appendBogus();
					}
					...
					if (!S.blockLimit.is('body')) {
						var T;
						if (L != 2)
							T = K.append(F.document.createElement(L == 1 ? 'p' : 'div'));
						else
							T = K;
						if (!c)
							T.appendBogus();
				...
				j.add('wysiwygarea', {requires: ['editingblock'], init: function(E) {
						...
						E.on('editingBlockReady', function() {
							...
							function W(X) {
								...
								if (c) {
									...
									setTimeout(function() {
										if (b.gecko && b.version >= 10900 || b.opera)
											Y.$.body.contentEditable = ab;
										else if (b.webkit)
											Y.$.body.parentNode.contentEditable = ab;
										else
											Y.$.designMode = ab ? 'off' : 'on';
									}, 0);
								...
								c && Y.on('click', function(ag) {
									...
									if (ah.is('input')) {
										...
										if (ai == 'submit' || ai == 'reset')
											ag.data.preventDefault();
									}
								});
								if (!(c || b.opera))
									Y.on('mousedown', function(ag) {
										var ah = ag.data.getTarget();
										if (ah.is('img', 'hr', 'input', 'textarea', 'select'))
											E.getSelection().selectElement(ah);
									});
								if (b.gecko)
								...
								Y.on('click', function(ag) {
									ag = ag.data;
									if (ag.getTarget().is('a') && ag.$.button != 2)
										ag.preventDefault();
								});
								if (b.webkit) {
									...
									Y.on('click', function(ag) {
										if (ag.data.getTarget().is('input', 'select'))
											ag.data.preventDefault();
									});
									Y.on('mouseup', function(ag) {
										if (ag.data.getTarget().is('input', 'textarea'))
											ag.data.preventDefault();
									});
								}
								var ac = c ? N : X;
								...
								ab && Y.on('keydown', function(ag) {
									var ah = ag.data.getKeystroke();
									if (ah in {8: 1, 46: 1}) {
										...
										if (aj) {
											...
											E.fire('saveSnapshot');
											ag.data.preventDefault();
										} else if ((am = al.block) && ak[ap ? 'checkStartOfBlock' : 'checkEndOfBlock']() && (ao = am[ap ? 'getPrevious' : 'getNext'](n)) && ao.is('table')) {
											E.fire('saveSnapshot');
											if (ak[ap ? 'checkEndOfBlock' : 'checkStartOfBlock']())
												am.remove();
											...
											E.fire('saveSnapshot');
											ak['moveToElementEdit' + (ap ? 'End' : 'Start')](ao);
											if (ak.checkStartOfBlock() && ak.checkEndOfBlock())
												ao.remove();
											else
												ak.select();
											E.fire('saveSnapshot');
								...
								if (E.contextMenu)
									E.contextMenu.addTarget(Y, E.config.browserContextMenuOnCtrl !== false);
								setTimeout(function() {
									E.fire('contentDom');
									if (R) {
										E.mode = 'wysiwyg';
									...
									if (E.config.disableObjectResizing)
										try {
							...
							E.addMode('wysiwyg', {load: function(X, Y, Z) {
									...
									if (Z)
										X = X.replace(/<!DOCTYPE[^>]*>/i, function(ad) {
											E.docType = aa = ad;
											return '';
									...
									if (Z) {
										...
										if (!/<html[\s|>]/.test(X))
											X = '<html>' + X + '</html>';
										if (!/<head[\s|>]/.test(X))
											X = X.replace(/<html[^>]*>/, '$&<head><title></title></head>');
										else if (!/<title[\s|>]/.test(X))
											X = X.replace(/<head[^>]*>/, '$&<title></title>');
										ac && (X = X.replace(/<head>/, '$&' + ac));
										X = X.replace(/<\/head\s*>/, ab + '$&');
										X = aa + X;
									} else
										X = Y.docType + '<html dir="' + Y.contentsLangDirection + '"' + ' lang="' + (Y.contentsLanguage || E.langCode) + '">' + '<head>' + '<title>' + G + '</title>' + ac + ab + '</head>' + '<body' + (Y.bodyId ? ' id="' + Y.bodyId + '"' : '') + (Y.bodyClass ? ' class="' + Y.bodyClass + '"' : '') + '>' + X + '</html>';
									if (b.gecko)
										X = X.replace(/<br \/>(?=\s*<\/(:?html|body)>)/, '$&<br type="_moz" />');
									X += V;
									this.onDispose();
									U(X);
								}, getData: function() {
									...
									if (b.gecko)
										ac = ac.replace(/<br>(?=\s*(:?$|<\/body>))/, '');
									if (E.dataProcessor)
										ac = E.dataProcessor.toDataFormat(ac, F);
									if (X.ignoreEmptyParagraph)
										ac = ac.replace(m, function(ad, ae) {
											return ae;
										});
									if (aa)
										ac = aa + '\n' + ac;
						...
						if (a.document.$.documentMode >= 8) {
							...
							var K = [];
							for (var L in f.$removeEmpty)
								K.push('html.CSS1Compat ' + L + '[contenteditable=false]');
						...
						} else if (c && b.version < 8 && E.config.contentsLangDirection == 'ltr')
							E.addCss('body{margin-right:0;}');
			...
			(function() {
				...
				function o() {
					var U = this;
					var Q = U._.currentTabId, R = U._.tabIdList.length, S = e.indexOf(U._.tabIdList, Q) + R;
					for (var T = S - 1; T > S - R; T--) {
						if (n.call(U, U._.tabIdList[T % R]))
							return U._.tabIdList[T % R];
					}
					return null;
				}
				;
				function p() {
					var U = this;
					var Q = U._.currentTabId, R = U._.tabIdList.length, S = e.indexOf(U._.tabIdList, Q);
					for (var T = S + 1; T < S + R; T++) {
						if (n.call(U, U._.tabIdList[T % R]))
							return U._.tabIdList[T % R];
					}
				...
				function r(Q, R) {
					var T = this;
					var S = T.getInputElement();
					if (S)
						Q ? S.removeAttribute('aria-invalid') : S.setAttribute('aria-invalid', true);
					if (!Q)
						if (T.select)
							T.select();
						else
							T.focus();
					R && alert(R);
					T.fire('validated', {valid: Q, msg: R});
				}
				;
				function s() {
					var Q = this.getInputElement();
					Q && Q.removeAttribute('aria-invalid');
				}
				;
				a.dialog = function(Q, R) {
					var S = a.dialog._.dialogDefinitions[R], T = e.clone(u), U = Q.config.dialog_buttonsOrder || 'OS', V = Q.lang.dir, W = {}, X, Y, Z;
					if (U == 'OS' && b.mac || U == 'rtl' && V == 'ltr' || U == 'ltr' && V == 'rtl')
						T.buttons.reverse();
					S = e.extend(S(Q), T);
					S = e.clone(S);
					S = new y(this, S);
					var aa = a.document, ab = Q.theme.buildDialog(Q);
					this._ = {editor: Q, element: ab.element, name: R, contentSize: {width: 0, height: 0}, size: {width: 0, height: 0}, contents: {}, buttons: {}, accessKeyMap: {}, tabs: {}, tabIdList: [], currentTabId: null, currentTabIndex: null, pageCount: 0, lastTab: null, tabBarMode: false, focusList: [], currentFocusIndex: 0, hasFocus: false};
					this.parts = ab.parts;
					e.setTimeout(function() {
						Q.fire('ariaWidget', this.parts.contents);
					}, 0, this);
					var ac = {position: b.ie6Compat ? 'absolute' : 'fixed', top: 0, visibility: 'hidden'};
					ac[V == 'rtl' ? 'right' : 'left'] = 0;
					this.parts.dialog.setStyles(ac);
					a.event.call(this);
					this.definition = S = a.fire('dialogDefinition', {name: R, definition: S}, Q).definition;
					if (!('removeDialogTabs' in Q._) && Q.config.removeDialogTabs) {
						var ad = Q.config.removeDialogTabs.split(';');
						...
						Q._.removeDialogTabs = W;
					}
					if (Q._.removeDialogTabs && (W = Q._.removeDialogTabs[R]))
						for (X = 0; X < W.length; X++)
							S.removeContents(W[X]);
					if (S.onLoad)
						this.on('load', S.onLoad);
					if (S.onShow)
						this.on('show', S.onShow);
					if (S.onHide)
						this.on('hide', S.onHide);
					if (S.onOk)
						this.on('ok', function(aq) {
							Q.fire('saveSnapshot');
							setTimeout(function() {
								Q.fire('saveSnapshot');
							}, 0);
							if (S.onOk.call(this, aq) === false)
								aq.data.hide = false;
						});
					if (S.onCancel)
						this.on('cancel', function(aq) {
							if (S.onCancel.call(this, aq) === false)
								aq.data.hide = false;
						});
					var ag = this, ah = function(aq) {
						var ar = ag._.contents, as = false;
						for (var at in ar)
							for (var au in ar[at]) {
								as = aq.call(this, ar[at][au]);
								if (as)
									return;
							}
					};
					this.on('ok', function(aq) {
						ah(function(ar) {
							if (ar.validate) {
								var as = ar.validate(this), at = typeof as == 'string' || as === false;
								if (at) {
									aq.data.hide = false;
									aq.stop();
								}
								r.call(ar, !at, typeof as == 'string' ? as : undefined);
								return at;
							}
						});
					}, this, null, 0);
					this.on('cancel', function(aq) {
						ah(function(ar) {
							if (ar.isChanged()) {
								if (!confirm(Q.lang.common.confirmCancel))
									aq.data.hide = false;
								return true;
							}
						});
					}, this, null, 0);
					this.parts.close.on('click', function(aq) {
						if (this.fire('cancel', {hide: true}).hide !== false)
							this.hide();
						aq.data.preventDefault();
					}, this);
					function ai() {
						var aq = ag._.focusList;
						aq.sort(function(at, au) {
							if (at.tabIndex != au.tabIndex)
								return au.tabIndex - at.tabIndex;
							else
								return at.focusIndex - au.focusIndex;
						});
						var ar = aq.length;
					...
					;
					function aj(aq) {
						var ar = ag._.focusList;
						aq = aq || 0;
						if (ar.length < 1)
							return;
						var as = ag._.currentFocusIndex;
						try {
							ar[as].getInputElement().$.blur();
						} catch (av) {
						}
						var at = (as + aq + ar.length) % ar.length, au = at;
						while (aq && !ar[au].isFocusable()) {
							au = (au + aq + ar.length) % ar.length;
							if (au == at)
								break;
						}
						ar[au].focus();
					...
					function ak(aq) {
						var ax = this;
						if (ag != a.dialog._.currentTop)
							return;
						var ar = aq.data.getKeystroke(), as = Q.lang.dir == 'rtl', at;
						Y = Z = 0;
						if (ar == 9 || ar == 2228224 + 9) {
							var au = ar == 2228224 + 9;
							if (ag._.tabBarMode) {
								var av = au ? o.call(ag) : p.call(ag);
								ag.selectPage(av);
								ag._.tabs[av][0].focus();
							} else
								aj(au ? -1 : 1);
							Y = 1;
						} else if (ar == 4456448 + 121 && !ag._.tabBarMode && ag.getPageCount() > 1) {
							ag._.tabBarMode = true;
							ag._.tabs[ag._.currentTabId][0].focus();
							Y = 1;
						} else if ((ar == 37 || ar == 39) && ag._.tabBarMode) {
							av = ar == (as ? 39 : 37) ? o.call(ag) : p.call(ag);
							ag.selectPage(av);
							ag._.tabs[av][0].focus();
							Y = 1;
						} else if ((ar == 13 || ar == 32) && ag._.tabBarMode) {
							ax.selectPage(ax._.currentTabId);
							ax._.tabBarMode = false;
							ax._.currentFocusIndex = -1;
							aj(1);
							Y = 1;
						} else if (ar == 13) {
							var aw = aq.data.getTarget();
							if (!aw.is('a', 'button', 'select') && (!aw.is('input') || aw.$.type != 'button')) {
								at = ax.getButton('ok');
								at && e.setTimeout(at.click, 0, at);
								Y = 1;
							}
							Z = 1;
						} else if (ar == 27) {
							at = ax.getButton('cancel');
							if (at)
								e.setTimeout(at.click, 0, at);
							else if (ax.fire('cancel', {hide: true}).hide !== false)
								ax.hide();
							Z = 1;
						} else
							return;
					...
					function al(aq) {
						if (Y)
							aq.data.preventDefault(1);
						else if (Z)
							aq.data.stopPropagation();
					}
					;
					var am = this._.element;
					this.on('show', function() {
						am.on('keydown', ak, this);
						if (b.opera || b.gecko)
							am.on('keypress', al, this);
					});
					this.on('hide', function() {
						am.removeListener('keydown', ak);
						if (b.opera || b.gecko)
							am.removeListener('keypress', al);
						ah(function(aq) {
							s.apply(aq);
						});
					});
					this.on('iframeAdded', function(aq) {
						var ar = new g(aq.data.iframe.$.contentWindow.document);
						ar.on('keydown', ak, this, null, 0);
					});
					this.on('show', function() {
						var au = this;
						ai();
						if (Q.config.dialog_startupFocusTab && ag._.pageCount > 1) {
							ag._.tabBarMode = true;
							ag._.tabs[ag._.currentTabId][0].focus();
						} else if (!au._.hasFocus) {
							au._.currentFocusIndex = -1;
							if (S.onFocus) {
								var aq = S.onFocus.call(au);
								aq && aq.focus();
							} else
								aj(1);
							if (au._.editor.mode == 'wysiwyg' && c) {
								var ar = Q.document.$.selection, as = ar.createRange();
								if (as)
									if (as.parentElement && as.parentElement().ownerDocument == Q.document.$ || as.item && as.item(0).ownerDocument == Q.document.$) {
										var at = document.body.createTextRange();
										at.moveToElementText(au.getElement().getFirst().$);
										at.collapse(true);
										at.select();
									}
							}
					...
					if (b.ie6Compat)
						this.on('load', function(aq) {
							var ar = this.getElement(), as = ar.getFirst();
							as.remove();
							as.appendTo(ar);
						}, this);
					A(this);
					...
					for (X = 0; X < S.contents.length; X++) {
						var an = S.contents[X];
						an && this.addPage(an);
					}
					this.parts.tabs.on('click', function(aq) {
						var at = this;
						var ar = aq.data.getTarget();
						if (ar.hasClass('cke_dialog_tab')) {
							var as = ar.$.id;
							at.selectPage(as.substring(4, as.lastIndexOf('_')));
							if (at._.tabBarMode) {
								at._.tabBarMode = false;
					...
					var ao = [], ap = a.dialog._.uiElementBuilders.hbox.build(this, {type: 'hbox', className: 'cke_dialog_footer_buttons', widths: [], children: S.buttons}, ao).getChild();
					this.parts.footer.setHtml(ao.join(''));
					for (X = 0; X < ap.length; X++)
						this._.buttons[ap[X].id] = ap[X];
				};
				function t(Q, R, S) {
					this.element = R;
					this.focusIndex = S;
					...
					R.on('keydown', function(T) {
						if (T.data.getKeystroke() in {32: 1, 13: 1})
							this.fire('click');
					});
					R.on('focus', function() {
						this.fire('mouseover');
				...
				a.dialog.prototype = {destroy: function() {
						...
						return function(Q, R) {
							var S = this;
							if (S._.contentSize && S._.contentSize.width == Q && S._.contentSize.height == R)
								return;
							a.dialog.fire('resize', {dialog: S, skin: S._.editor.skinName, width: Q, height: R}, S._.editor);
							S.fire('resize', {skin: S._.editor.skinName, width: Q, height: R}, S._.editor);
							if (S._.editor.lang.dir == 'rtl' && S._.position)
								S._.position.x = a.document.getWindow().getViewPaneSize().width - S._.contentSize.width - parseInt(S._.element.getFirst().getStyle('right'), 10);
							S._.contentSize = {width: Q, height: R};
						};
					})(), getSize: function() {
						var Q = this._.element.getFirst();
						return{width: Q.$.offsetWidth || 0, height: Q.$.offsetHeight || 0};
					}, move: (function() {
						var Q;
						return function(R, S, T) {
							var aa = this;
							var U = aa._.element.getFirst(), V = aa._.editor.lang.dir == 'rtl';
							if (Q === undefined)
								Q = U.getComputedStyle('position') == 'fixed';
							if (Q && aa._.position && aa._.position.x == R && aa._.position.y == S)
								return;
							aa._.position = {x: R, y: S};
							if (!Q) {
							...
							if (V) {
								var X = aa.getSize(), Y = a.document.getWindow().getViewPaneSize();
								R = Y.width - X.width - R;
							}
							var Z = {top: (S > 0 ? S : 0) + 'px'};
						...
						var Q = this._.element, R = this.definition;
						if (!(Q.getParent() && Q.getParent().equals(a.document.getBody())))
						...
						var Q = arguments;
						this.foreach(function(R) {
							if (c && this._.currentFocusIndex == R.focusIndex)
								R.getInputElement().$.blur();
							if (R.commit)
								R.commit.apply(R, Q);
						});
					}, hide: function() {
						...
						if (!this._.parentDialog) {
							...
							Q.removeListener('keydown', K);
							Q.removeListener(b.opera ? 'keypress' : 'keyup', L);
							var S = this._.editor;
							S.focus();
							if (S.mode == 'wysiwyg' && c) {
						...
						var ac = this;
						var R = [], S = Q.label ? ' title="' + e.htmlEncode(Q.label) + '"' : '', T = Q.elements, U = a.dialog._.uiElementBuilders.vbox.build(ac, {type: 'vbox', className: 'cke_dialog_page_contents', children: Q.elements, expand: !!Q.expand, padding: Q.padding, style: Q.style || 'width: 100%;height:100%'}, R), V = h.createFromHtml(R.join(''));
						V.setAttribute('role', 'tabpanel');
						var W = b, X = 'cke_' + Q.id + '_' + e.getNextNumber(), Y = h.createFromHtml(['<a class="cke_dialog_tab"', ac._.pageCount > 0 ? ' cke_last' : 'cke_first', S, !!Q.hidden ? ' style="display:none"' : '', ' id="', X, '"', W.gecko && W.version >= 10900 && !W.hc ? '' : ' href="javascript:void(0)"', ' tabIndex="-1"', ' hidefocus="true"', ' role="tab">', Q.label, '</a>'].join(''));
						...
						ac._.lastTab = Y;
						ac.updateStyle();
						var Z = ac._.contents[Q.id] = {}, aa, ab = U.getChild();
						while (aa = ab.shift()) {
							Z[aa.id] = aa;
							if (typeof aa.getChild == 'function')
								ab.push.apply(ab, aa.getChild());
						}
						...
						if (Q.accessKey) {
							M(ac, ac, 'CTRL+' + Q.accessKey, P, O);
							ac._.accessKeyMap['CTRL+' + Q.accessKey] = Q.id;
						}
					}, selectPage: function(Q) {
						if (this._.currentTabId == Q)
							return;
						if (this.fire('selectPage', {page: Q, currentPage: this._.currentTabId}) === true)
							return;
						for (var R in this._.tabs) {
							var S = this._.tabs[R][0], T = this._.tabs[R][1];
							if (R != Q) {
						...
						if (!R || S._.pageCount == 1 || !R.isVisible())
							return;
						else if (Q == S._.currentTabId)
							S.selectPage(o.call(S));
						R.hide();
						S._.pageCount--;
						S.updateStyle();
					}, showPage: function(Q) {
						...
						if (typeof R == 'undefined') {
							...
							T._.focusList.push(new t(T, Q, R));
						} else {
							T._.focusList.splice(R, 0, new t(T, Q, R));
							for (var S = R + 1; S < T._.focusList.length; S++)
								T._.focusList[S].focusIndex++;
						}
					}};
				e.extend(a.dialog, {add: function(Q, R) {
						if (!this._.dialogDefinitions[Q] || typeof R == 'function')
							this._.dialogDefinitions[Q] = R;
					}, exists: function(Q) {
						...
						var Q = function(R, S) {
							S = S || {};
							return e.extend({id: 'ok', type: 'button', label: R.lang.common.ok, 'class': 'cke_dialog_ui_button_ok', onClick: function(T) {
									var U = T.data.dialog;
									if (U.fire('ok', {hide: true}).hide !== false)
										U.hide();
								}}, S, true);
						};
						Q.type = 'button';
						...
						var Q = function(R, S) {
							S = S || {};
							return e.extend({id: 'cancel', type: 'button', label: R.lang.common.cancel, 'class': 'cke_dialog_ui_button_cancel', onClick: function(T) {
									var U = T.data.dialog;
									if (U.fire('cancel', {hide: true}).hide !== false)
										U.hide();
								}}, S, true);
						};
						Q.type = 'button';
				...
				var u = {resizable: 3, minWidth: 600, minHeight: 400, buttons: [a.dialog.okButton, a.dialog.cancelButton]}, v = function(Q, R, S) {
				...
				y.prototype = {getContents: function(Q) {
						return v(this.contents, Q);
					}, getButton: function(Q) {
						return v(this.buttons, Q);
					}, addContents: function(Q, R) {
						return w(this.contents, Q, R);
					}, addButton: function(Q, R) {
						return w(this.buttons, Q, R);
				...
				z.prototype = {get: function(Q) {
						return v(this.elements, Q, 'children');
					}, add: function(Q, R) {
						return w(this.elements, Q, R, 'children');
					}, remove: function(Q) {
						x(this.elements, Q, 'children');
					}};
				function A(Q) {
					var R = null, S = null, T = Q.getElement().getFirst(), U = Q.getParentEditor(), V = U.config.dialog_magnetDistance, W = U.skin.margins || [0, 0, 0, 0];
					if (typeof V == 'undefined')
						V = 20;
					function X(Z) {
						var aa = Q.getSize(), ab = a.document.getWindow().getViewPaneSize(), ac = Z.data.$.screenX, ad = Z.data.$.screenY, ae = ac - R.x, af = ad - R.y, ag, ah;
						R = {x: ac, y: ad};
						S.x += ae;
						S.y += af;
						if (S.x + W[3] < V)
							ag = -W[3];
						else if (S.x - W[1] > ab.width - aa.width - V)
							ag = ab.width - aa.width + (U.lang.dir == 'rtl' ? 0 : W[1]);
						else
							ag = S.x;
						if (S.y + W[0] < V)
							ah = -W[0];
						else if (S.y - W[2] > ab.height - aa.height - V)
							ah = ab.height - aa.height + W[2];
						else
							ah = S.y;
						Q.move(ag, ah, 1);
					...
					function Y(Z) {
						...
						if (b.ie6Compat) {
							var aa = E.getChild(0).getFrameDocument();
					...
					Q.parts.title.on('mousedown', function(Z) {
						R = {x: Z.data.$.screenX, y: Z.data.$.screenY};
				...
				;
				function B(Q) {
					var R = Q.definition, S = R.resizable;
					if (S == 0)
						return;
					var T = Q.getParentEditor(), U, V, W, X, Y, Z, aa = e.addFunction(function(ad) {
						Y = Q.getSize();
						var ae = Q.parts.contents, af = ae.$.getElementsByTagName('iframe').length;
						if (af) {
							Z = h.createFromHtml('<div class="cke_dialog_resize_cover" style="height: 100%; position: absolute; width: 100%;"></div>');
							ae.append(Z);
						}
						V = Y.height - Q.parts.contents.getSize('height', !(b.gecko || b.opera || c && b.quirks));
						U = Y.width - Q.parts.contents.getSize('width', 1);
						X = {x: ad.screenX, y: ad.screenY};
						W = a.document.getWindow().getViewPaneSize();
						a.document.on('mousemove', ab);
						a.document.on('mouseup', ac);
						if (b.ie6Compat) {
							var ag = E.getChild(0).getFrameDocument();
							ag.on('mousemove', ab);
							ag.on('mouseup', ac);
						}
					...
					Q.on('load', function() {
						...
						var ae = h.createFromHtml('<div class="cke_resizer' + ad + ' cke_resizer_' + T.lang.dir + '"' + ' title="' + e.htmlEncode(T.lang.resize) + '"' + ' onmousedown="CKEDITOR.tools.callFunction(' + aa + ', event )"></div>');
					...
					function ab(ad) {
						var ae = T.lang.dir == 'rtl', af = (ad.data.$.screenX - X.x) * (ae ? -1 : 1), ag = ad.data.$.screenY - X.y, ah = Y.width, ai = Y.height, aj = ah + af * (Q._.moved ? 1 : 2), ak = ai + ag * (Q._.moved ? 1 : 2), al = Q._.element.getFirst(), am = ae && al.getComputedStyle('right'), an = Q.getPosition();
						if (an.y + ak > W.height)
							ak = W.height - an.y;
						if ((ae ? am : an.x) + aj > W.width)
							aj = W.width - (ae ? am : an.x);
						if (S == 1 || S == 3)
							ah = Math.max(R.minWidth || 0, aj - U);
						if (S == 2 || S == 3)
							ai = Math.max(R.minHeight || 0, ak - V);
						Q.resize(ah, ai);
						if (!Q._.moved)
							Q.layout();
					...
					function ac() {
						...
						if (b.ie6Compat) {
							var ad = E.getChild(0).getFrameDocument();
				...
				;
				function G(Q) {
					var R = a.document.getWindow(), S = Q.config, T = S.dialog_backgroundCoverColor || 'white', U = S.dialog_backgroundCoverOpacity, V = S.baseFloatZIndex, W = e.genKey(T, U, V), X = D[W];
					if (!X) {
						var Y = ['<div tabIndex="-1" style="position: ', b.ie6Compat ? 'absolute' : 'fixed', '; z-index: ', V, '; top: 0px; left: 0px; ', !b.ie6Compat ? 'background-color: ' + T : '', '" class="cke_dialog_background_cover">'];
						if (b.ie6Compat) {
							var Z = b.isCustomDomain(), aa = "<html><body style=\\'background-color:" + T + ";\\'></body></html>";
							Y.push('<iframe hidefocus="true" frameborder="0" id="cke_dialog_background_iframe" src="javascript:');
							Y.push('void((function(){document.open();' + (Z ? "document.domain='" + document.domain + "';" : '') + "document.write( '" + aa + "' );" + 'document.close();' + '})())');
							Y.push('" style="position:absolute;left:0;top:0;width:100%;height: 100%;progid:DXImageTransform.Microsoft.Alpha(opacity=0)"></iframe>');
						}
						Y.push('</div>');
					...
					var ab = function() {
						...
						X.setStyles({width: ae.width + 'px', height: ae.height + 'px'});
					}, ac = function() {
						var ae = R.getScrollPosition(), af = a.dialog._.currentTop;
						X.setStyles({left: ae.x + 'px', top: ae.y + 'px'});
						if (af)
							do {
				...
				;
				var J = {}, K = function(Q) {
					var R = Q.data.$.ctrlKey || Q.data.$.metaKey, S = Q.data.$.altKey, T = Q.data.$.shiftKey, U = String.fromCharCode(Q.data.$.keyCode), V = J[(R ? 'CTRL+' : '') + (S ? 'ALT+' : '') + (T ? 'SHIFT+' : '') + U];
					if (!V || !V.length)
						return;
					V = V[V.length - 1];
				...
				}, M = function(Q, R, S, T, U) {
					var V = J[S] || (J[S] = []);
					V.push({uiElement: Q, dialog: R, key: S, keyup: U || Q.accessKeyUp, keydown: T || Q.accessKeyDown});
				}, N = function(Q) {
					for (var R in J) {
						var S = J[R];
						for (var T = S.length - 1; T >= 0; T--) {
							if (S[T].dialog == Q || S[T].uiElement == Q)
								S.splice(T, 1);
						}
						if (S.length === 0)
							delete J[R];
					...
					if (Q._.accessKeyMap[R])
						Q.selectPage(Q._.accessKeyMap[R]);
				}, P = function(Q, R) {
				};
				(function() {
					k.dialog = {uiElement: function(Q, R, S, T, U, V, W) {
							if (arguments.length < 4)
								return;
							var X = (T.call ? T(R) : T) || 'div', Y = ['<', X, ' '], Z = (U && U.call ? U(R) : U) || {}, aa = (V && V.call ? V(R) : V) || {}, ab = (W && W.call ? W.call(this, Q, R) : W) || '', ac = this.domId = aa.id || e.getNextId() + '_uiElement', ad = this.id = R.id, ae;
							aa.id = ac;
							var af = {};
							if (R.type)
								af['cke_dialog_ui_' + R.type] = 1;
							if (R.className)
								af[R.className] = 1;
							if (R.disabled)
								af.cke_disabled = 1;
							...
							if (R.align) {
								var aj = R.align;
								Z['margin-left'] = aj == 'left' ? 0 : 'auto';
								Z['margin-right'] = aj == 'right' ? 0 : 'auto';
							}
							for (ae in Z)
								ai.push(ae + ':' + Z[ae]);
							if (R.hidden)
								ai.push('display:none');
							for (ae = ai.length - 1; ae >= 0; ae--) {
								if (ai[ae] === '')
									ai.splice(ae, 1);
							}
							if (ai.length > 0)
								aa.style = (aa.style ? aa.style + '; ' : '') + ai.join('; ');
							for (ae in aa)
								Y.push(ae + '="' + e.htmlEncode(aa[ae]) + '" ');
							Y.push('>', ab, '</', X, '>');
							S.push(Y.join(''));
							(this._ || (this._ = {})).dialog = Q;
							if (typeof R.isChanged == 'boolean')
								this.isChanged = function() {
									return R.isChanged;
								};
							if (typeof R.isChanged == 'function')
								this.isChanged = R.isChanged;
							if (typeof R.setValue == 'function')
								this.setValue = e.override(this.setValue, function(al) {
									return function(am) {
										al.call(this, R.setValue.call(this, am));
									};
								});
							if (typeof R.getValue == 'function')
								this.getValue = e.override(this.getValue, function(al) {
									return function() {
										return R.getValue.call(this, al.call(this));
									};
								});
							a.event.implementOn(this);
							this.registerEvents(R);
							if (this.accessKeyUp && this.accessKeyDown && R.accessKey)
								M(this, Q, 'CTRL+' + R.accessKey);
							var ak = this;
							Q.on('load', function() {
								var al = ak.getInputElement();
								if (al) {
									var am = ak.type in {checkbox: 1, ratio: 1} && c && b.version < 8 ? 'cke_dialog_ui_focused' : '';
									al.on('focus', function() {
							...
							if (this.keyboardFocusable) {
								...
								this.on('focus', function() {
									Q._.currentFocusIndex = ak.focusIndex;
								});
							}
							e.extend(this, R);
							...
							if (arguments.length < 4)
								return;
							this._ || (this._ = {});
							var V = this._.children = R, W = U && U.widths || null, X = U && U.height || null, Y = {}, Z, aa = function() {
								var ac = ['<tbody><tr class="cke_dialog_ui_hbox">'];
								for (Z = 0; Z < S.length; Z++) {
									var ad = 'cke_dialog_ui_hbox_child', ae = [];
									if (Z === 0)
										ad = 'cke_dialog_ui_hbox_first';
									if (Z == S.length - 1)
										ad = 'cke_dialog_ui_hbox_last';
									ac.push('<td class="', ad, '" role="presentation" ');
									if (W) {
										if (W[Z])
											ae.push('width:' + m(W[Z]));
									} else
										ae.push('width:' + Math.floor(100 / S.length) + '%');
									if (X)
										ae.push('height:' + m(X));
									if (U && U.padding != undefined)
										ae.push('padding:' + m(U.padding));
									if (c && b.quirks && V[Z].align)
										ae.push('text-align:' + V[Z].align);
									if (ae.length > 0)
										ac.push('style="' + ae.join('; ') + '" ');
									ac.push('>', S[Z], '</td>');
								}
								ac.push('</tr></tbody>');
								return ac.join('');
							}, ab = {role: 'presentation'};
							U && U.align && (ab.align = U.align);
							k.dialog.uiElement.call(this, Q, U || {type: 'hbox'}, T, 'table', Y, ab, aa);
						}, vbox: function(Q, R, S, T, U) {
							if (arguments.length < 3)
								return;
							this._ || (this._ = {});
							var V = this._.children = R, W = U && U.width || null, X = U && U.heights || null, Y = function() {
								var Z = ['<table role="presentation" cellspacing="0" border="0" '];
								Z.push('style="');
								if (U && U.expand)
									Z.push('height:100%;');
								Z.push('width:' + m(W || '100%'), ';');
								Z.push('"');
								Z.push('align="', e.htmlEncode(U && U.align || (Q.getParentEditor().lang.dir == 'ltr' ? 'left' : 'right')), '" ');
								Z.push('><tbody>');
								for (var aa = 0; aa < S.length; aa++) {
									var ab = [];
									Z.push('<tr><td role="presentation" ');
									if (W)
										ab.push('width:' + m(W || '100%'));
									if (X)
										ab.push('height:' + m(X[aa]));
									else if (U && U.expand)
										ab.push('height:' + Math.floor(100 / S.length) + '%');
									if (U && U.padding != undefined)
										ab.push('padding:' + m(U.padding));
									if (c && b.quirks && V[aa].align)
										ab.push('text-align:' + V[aa].align);
									if (ab.length > 0)
										Z.push('style="', ab.join('; '), '" ');
									Z.push(' class="cke_dialog_ui_vbox_child">', S[aa], '</td></tr>');
								}
								Z.push('</tbody></table>');
								return Z.join('');
							};
							k.dialog.uiElement.call(this, Q, U || {type: 'vbox'}, T, 'div', null, {role: 'presentation'}, Y);
						}};
				})();
				k.dialog.uiElement.prototype = {getElement: function() {
						...
						for (var U in Q) {
							if (!(S = U.match(R)))
								continue;
							if (this.eventProcessors[U])
								this.eventProcessors[U].call(this, this._.dialog, Q[U]);
							else
								T(this, this._.dialog, S[1].toLowerCase(), Q[U]);
						}
						return this;
					}, eventProcessors: {onLoad: function(Q, R) {
							Q.on('load', R, this);
						}, onShow: function(Q, R) {
							Q.on('show', R, this);
						}, onHide: function(Q, R) {
						...
						var Q = this.getElement(), R = this.getInputElement();
				...
				k.dialog.hbox.prototype = e.extend(new k.dialog.uiElement(), {getChild: function(Q) {
						var R = this;
						if (arguments.length < 1)
							return R._.children.concat();
						if (!Q.splice)
							Q = [Q];
						if (Q.length < 2)
							return R._.children[Q[0]];
						else
							return R._.children[Q[0]] && R._.children[Q[0]].getChild ? R._.children[Q[0]].getChild(Q.slice(1, Q.length)) : null;
					}}, true);
				k.dialog.vbox.prototype = new k.dialog.hbox();
				(function() {
					var Q = {build: function(R, S, T) {
							var U = S.children, V, W = [], X = [];
				...
				a.dialogCommand.prototype = {exec: function(Q) {
						b.opera ? e.setTimeout(function() {
							Q.openDialog(this.dialogName);
						}, 0, this) : Q.openDialog(this.dialogName);
					}, canUndo: false, editorFocus: c || b.webkit};
				(function() {
					var Q = /^([a]|[^a])+$/, R = /^\d*$/, S = /^\d*(?:\.\d+)?$/, T = /^(((\d*(\.\d+))|(\d*))(px|\%)?)?$/, U = /^(((\d*(\.\d+))|(\d*))(px|em|ex|in|cm|mm|pt|pc|\%)?)?$/i, V = /^(\s*[\w-]+\s*:\s*[^:;]+(?:;|$))*$/;
					a.VALIDATE_OR = 1;
					a.VALIDATE_AND = 2;
					a.dialog.validate = {functions: function() {
							var W = arguments;
							return function() {
								var X = this && this.getValue ? this.getValue() : W[0], Y = undefined, Z = 2, aa = [], ab;
								for (ab = 0; ab < W.length; ab++) {
									if (typeof W[ab] == 'function')
										aa.push(W[ab]);
									else
										break;
								}
								if (ab < W.length && typeof W[ab] == 'string') {
									Y = W[ab];
									ab++;
								}
								if (ab < W.length && typeof W[ab] == 'number')
									Z = W[ab];
								var ac = Z == 2 ? true : false;
							...
							return function() {
								var Y = this && this.getValue ? this.getValue() : arguments[0];
								return!W.test(Y) ? X : true;
							};
						}, notEmpty: function(W) {
							return this.regex(Q, W);
						}, integer: function(W) {
							return this.regex(R, W);
						}, number: function(W) {
							return this.regex(S, W);
						}, cssLength: function(W) {
							return this.functions(function(X) {
								return U.test(e.trim(X));
							}, W);
						}, htmlLength: function(W) {
							return this.functions(function(X) {
								return T.test(e.trim(X));
							}, W);
						}, inlineStyle: function(W) {
							return this.functions(function(X) {
								return V.test(e.trim(X));
							}, W);
						}, equals: function(W, X) {
							return this.functions(function(Y) {
								return Y == W;
							}, X);
						}, notEqual: function(W, X) {
							return this.functions(function(Y) {
								return Y != W;
							}, X);
						}};
					a.on('instanceDestroyed', function(W) {
						if (e.isEmpty(a.instances)) {
							var X;
							while (X = a.dialog._.currentTop)
								X.hide();
							I();
						}
						var Y = W.editor._.storedDialogs;
						for (var Z in Y)
							Y[Z].destroy();
					});
				})();
				e.extend(a.editor.prototype, {openDialog: function(Q, R) {
						if (this.mode == 'wysiwyg' && c) {
							var S = this.getSelection();
							S && S.lock();
						}
						var T = a.dialog._.dialogDefinitions[Q], U = this.skin.dialog;
						if (a.dialog._.currentTop === null)
							G(this);
						if (typeof T == 'function' && U._isLoaded) {
							var V = this._.storedDialogs || (this._.storedDialogs = {}), W = V[Q] || (V[Q] = new a.dialog(this, Q));
							R && R.call(W, W);
							W.show();
							return W;
						} else if (T == 'failed') {
						...
						var X = this;
						function Y(aa) {
							var ab = a.dialog._.dialogDefinitions[Q], ac = X.skin.dialog;
							if (!ac._isLoaded || Z && typeof aa == 'undefined')
								return;
							if (typeof ab != 'function')
								a.dialog._.dialogDefinitions[Q] = 'failed';
							X.openDialog(Q, R);
						}
						;
						if (typeof T == 'string') {
			...
			(function() {
				...
				a.style.prototype = {apply: function(T) {
						...
						switch (Y.type) {
							...
							case 2:
								...
								for (var V = 0, W; V < U.length; V++) {
									W = U[V];
									if (Y.type == 2 && (W == T.block || W == T.blockLimit))
										continue;
									if (Y.type == 3) {
										var X = W.getName();
										if (!(typeof Y.element == 'string' ? X == Y.element : X in Y.element))
											continue;
									}
									if (Y.checkElementRemovable(W, true))
										return true;
								}
						}
						return false;
						...
						if (!T || !V.ignoreReadonly && T.isReadOnly())
							return false;
						var W, X = T.getName();
						...
						if (V) {
							...
							for (var Y = 0; Y < W.length; Y++) {
								...
								if (Z) {
									var aa = W[Y][1];
									if (aa === null || typeof aa == 'string' && Z == aa || aa.test(Z))
										return true;
								}
							}
						}
				...
				function t(T) {
					...
					if (aa == undefined)
						aa = U.getCustomData('cke_includeReadonly');
					...
					if (!Z) {
						var ah = s(ad), ai = s(ae);
						if (ah)
							af = ah.getNextSourceNode(true);
						if (ai)
							ae = ai;
					}
					if (af.getPosition(ae) == 2)
						af = 0;
					while (af) {
						var aj = false;
						if (af.equals(ae)) {
							...
							if (!al || ab[al] && !an && (!am || aa) && (af.getPosition(ae) | 4 | 0 | 8) == 4 + 0 + 8 && (!X.childRule || X.childRule(af))) {
								...
								if (ao && ((ao.getDtd() || f.span)[W] || Y) && (!X.parentRule || X.parentRule(ao))) {
									...
									if (ak == 3 || am || ak == 1 && !af.getChildCount()) {
										var ap = af, aq;
										while ((aj = !ap.getNext(q)) && (aq = ap.getParent(), ab[aq.getName()]) && (aq.getPosition(ad) | 2 | 0 | 8) == 2 + 0 + 8 && (!X.childRule || X.childRule(aq)))
											ap = aq;
										ag.setEndAfter(ap);
									}
								} else
							...
							while (ar && at) {
								if (at.getName() == W) {
									for (av in X.attributes) {
										if (au.blockedAttrs[av] || !(ax = at.getAttribute(aw)))
											continue;
										if (ar.getAttribute(av) == ax)
											au.attrs[av] = 1;
										else
											au.blockedAttrs[av] = 1;
									}
									for (aw in X.styles) {
										if (au.blockedStyles[aw] || !(ax = at.getStyle(aw)))
											continue;
										if (ar.getStyle(aw) == ax)
											au.styles[aw] = 1;
										else
											au.blockedStyles[aw] = 1;
									}
								}
								at = at.getParent();
							}
							for (av in au.attrs)
								ar.removeAttribute(av);
							for (aw in au.styles)
								ar.removeStyle(aw);
							if (as && !ar.hasAttributes())
								ar = null;
							if (ar) {
						...
						for (var Y = 0, Z; Y < W.elements.length && (Z = W.elements[Y]); Y++) {
							...
							if (this.checkElementRemovable(Z)) {
								...
								if (T.collapsed && (T.checkBoundaryOfElement(Z, 2) || (aa = T.checkBoundaryOfElement(Z, 1)))) {
									...
									Z.mergeSiblings();
									if (Z.getName() == this.element)
										F(this, Z);
									else
										H(Z, N(this)[Z.getName()]);
								}
							}
						}
						if (X) {
				...
				function w(T) {
					...
					if (Y)
						for (var Z in Y)
							V.removeAttribute(Z, Y[Z]);
					if (X.styles)
						for (var aa in X.styles) {
							if (!X.styles.hasOwnProperty(aa))
								continue;
							V.removeStyle(aa);
						}
				}
				...
				function A(T) {
					var U;
					if (!((U = T.getPrevious(r)) && U.is && U.is('pre')))
						return;
					var V = C(U.getHtml(), /\n$/, '') + '\n\n' + C(T.getHtml(), /^\n/, '');
					if (c)
				...
				function F(T, U) {
					...
					for (var aa in W) {
						if ((aa == 'class' || T._.definition.fullMatch) && U.getAttribute(aa) != O(aa, W[aa]))
							continue;
						Z = U.hasAttribute(aa);
						U.removeAttribute(aa);
					}
					for (var ab in X) {
						if (T._.definition.fullMatch && U.getStyle(ab) != O(ab, X[ab], true))
							continue;
						Z = Z || !!U.getStyle(ab);
						U.removeStyle(ab);
					}
					H(U, Y, m[U.getName()]);
					if (Z)
						!f.$block[U.getName()] || T._.enterMode == 2 && !U.hasAttributes() ? I(U) : U.renameNode(T._.enterMode == 1 ? 'p' : 'div');
				}
				;
				function G(T, U) {
					...
					if (W)
						for (var X = 0; X < W.length; X++) {
							...
							if (Z = T.getAttribute(Y)) {
								var aa = W[X][1];
								if (aa === null || aa.test && aa.test(Z) || typeof aa == 'string' && Z == aa)
									T.removeAttribute(Y);
							}
						}
					if (!V)
				...
				function L(T, U) {
					for (var V in T)
						T[V] = T[V].replace(p, function(W, X) {
							return U[X];
						});
				}
				;
			...
			(function() {
				...
				function r(s, t, u) {
					...
					while (!o(v))
						v = v.getNextSourceNode(t, null, u);
					return v;
				}
				;
				m.prototype = {getNextParagraph: function(s) {
						...
						if (!t) {
							...
							t = M.block;
							if (!t && !S.enforceRealBlocks && O[N.getName()] && u.checkStartOfBlock() && u.checkEndOfBlock())
								t = N;
								...
								S._.nextNode = t.equals(B) ? null : r(u.getBoundaryNodes().endNode, 1, B);
						}
						if (x) {
							var Q = t.getPrevious();
							if (Q && Q.type == 1)
								if (Q.getName() == 'br')
									Q.remove();
								else if (Q.getLast() && Q.getLast().$.nodeName.toLowerCase() == 'br')
									Q.getLast().remove();
						}
						if (y) {
							var R = t.getLast();
							if (R && R.type == 1 && R.getName() == 'br')
								if (c || R.getPrevious(o) || R.getNext(o))
									R.remove();
						}
						if (!S._.nextNode)
							S._.nextNode = v || t.equals(B) || !B ? null : r(t, 1, B);
						return t;
					}};
								...
								var E = e.bind(function() {
									...
									if (R < O)
										if (S > O)
											...
											C = C - L.bottom + M.height;
										else
											C -= L.top;
									if (c) {
										var T = new h(w.$.offsetParent), U = T;
										if (U.getName() == 'html')
											U = U.getDocument().getBody();
										if (U.getComputedStyle('direction') == 'rtl')
											if (b.ie8Compat)
									...
									var V = w.getFirst(), W;
									if (W = V.getCustomData('activePanel'))
										W.onHide && W.onHide.call(this, 1);
			...
			(function() {
				...
				j.add('editingblock', {init: function(n) {
						if (!n.config.editingBlock)
							return;
						n.on('themeSpace', function(o) {
							if (o.data.space == 'contents')
								o.data.html += '<br>';
						});
						...
						n.on('getSnapshot', function(o) {
							if (n.mode)
								o.data = n.getMode().getSnapshotData();
						});
						n.on('loadSnapshot', function(o) {
							if (n.mode)
								n.getMode().loadSnapshotData(o.data);
						});
						...
						n.on('destroy', function() {
							var o = this;
							if (o.mode)
								o._.modes[o.mode].unload(o.getThemeSpace('contents'));
						});
					}});
				...
				a.editor.prototype.setMode = function(n) {
					...
					if (!s)
						throw '[CKEDITOR.editor.setMode] Unknown mode "' + n + '".';
					if (!q)
			...
			(function() {
				...
				function v(z) {
					var A = z && u(z);
					if (A)
						if (A.getCustomData('ready'))
							w(z);
						else
							A.setCustomData('ready', 1);
				}
				;
				function w(z) {
				...
				j.add('selection', {init: function(z) {
						if (b.webkit) {
							...
							function C() {
								...
								if (F) {
									...
									if (G.type == 'Caret' && G.anchorNode == F.$)
										B = 1;
									A = F.getText();
						...
						z.on('contentDom', function() {
							...
							if (c) {
								var H, I, J = 1;
								F.on('focusin', function(O) {
									if (O.data.$.srcElement.nodeName != 'BODY')
										return;
									var P = E.getCustomData('cke_locked_selection');
									if (P) {
										P.unlock(1);
										P.lock();
									} else if (H && J) {
								...
								F.on('beforedeactivate', function(O) {
									if (O.data.$.toElement)
										return;
									I = 0;
									J = 1;
								...
								var K;
								F.on('mousedown', function(O) {
									if (O.data.$.button == 2) {
										var P = z.document.$.selection;
										if (P.type == 'None')
											K = z.window.getScrollPosition();
									}
									M();
								});
								F.on('mouseup', function(O) {
									if (O.data.$.button == 2 && K) {
								...
								if ((b.ie7Compat || b.ie6Compat) && E.$.compatMode != 'BackCompat') {
									function L(O, P, Q) {
										try {
											O.moveToPoint(P, Q);
										} catch (R) {
									...
									;
									G.on('mousedown', function(O) {
										function P(R) {
											R = R.data.$;
											if (Q) {
												var S = F.$.createTextRange();
												L(S, R.x, R.y);
												Q.setEndPoint(Q.compareEndPoints('StartToStart', S) < 0 ? 'EndToEnd' : 'StartToStart', S);
												Q.select();
											}
										}
										;
										O = O.data.$;
										if (O.y < G.$.clientHeight && O.y > F.$.offsetTop + F.$.clientHeight && O.x < G.$.clientWidth) {
											var Q = F.$.createTextRange();
											L(Q, O.x, O.y);
											G.on('mousemove', P);
											G.on('mouseup', function(R) {
												G.removeListener('mousemove', P);
												R.removeListener();
												Q.select();
											});
										}
								...
								if (b.ie8)
									G.on('mouseup', function(O) {
										if (O.data.getTarget().getName() == 'html') {
											var P = a.document.$.selection, Q = P.createRange();
											if (P.type != 'None' && Q.parentElement().ownerDocument == E.$)
												Q.select();
										}
									});
								E.on('selectionchange', N);
								function M() {
									I = 0;
								}
								;
								function N(O) {
									if (I) {
										var P = z.document, Q = z.getSelection(), R = Q && Q.getNative();
										if (O && R && R.type == 'None')
											if (!P.$.queryCommandEnabled('InsertImage')) {
												e.setTimeout(N, 50, this, true);
												return;
											}
										var S;
										if (R && R.type && R.type != 'Control' && (S = R.createRange()) && (S = S.parentElement()) && (S = S.nodeName) && S.toLowerCase() in {input: 1, textarea: 1})
											return;
										H = R && Q.getRanges()[0];
										p.call(z);
									}
				...
				d.selection = function(z) {
					...
					if (c)
						try {
							...
							if (!B || B.item && B.item(0).ownerDocument != C.document.$ || B.parentElement && B.parentElement().ownerDocument != C.document.$)
								throw 0;
						} catch (D) {
							C.isInvalid = true;
						}
				...
				d.selection.prototype = {getNative: c ? function() {
						...
						try {
							...
							if (C == 'Control')
								A = 3;
							if (B.createRange().parentElement)
								A = 2;
						} catch (D) {
						}
						return z.type = A;
						...
						if (!B)
							...
							if (D == C.endContainer && D.nodeType == 1 && C.endOffset - C.startOffset == 1 && y[D.childNodes[C.startOffset].nodeName.toLowerCase()])
								A = 3;
						}
						return z.type = A;
					}, getRanges: (function() {
						var z = c ? (function() {
							...
							var B = function(C, D) {
								...
								if (M == -1 || M == G.length - 1 && N < 0) {
									...
									if (!O) {
										H = G[G.length - 1];
										if (H.nodeType != 3)
											return{container: E, offset: G.length};
										else
											return{container: H, offset: H.nodeValue.length};
									}
									var R = G.length;
									while (O > 0 && R > 0) {
										I = G[--R];
									...
									O = J.text.replace(/(\r\n|\r)/g, '\n').length;
									if (!O)
										return{container: E, offset: A(H) + (N > 0 ? 0 : 1)};
									while (O > 0)
										try {
							...
							return function() {
								...
								if (E == 2) {
									...
									if (F.endContainer.getPosition(F.startContainer) & 4 && F.endOffset <= F.startContainer.getIndex())
										F.collapse();
									return[F];
								} else if (E == 3) {
									var H = [];
						...
						return function(A) {
							...
							if (B.ranges && !A)
								...
								B.ranges = new d.rangeList(z.call(this));
							if (A) {
								var C = B.ranges;
						...
						var G = this;
						var z = G._.cache;
						if (z.startElement !== undefined)
							return z.startElement;
						var A, B = G.getNative();
						switch (G.getType()) {
							case 3:
								...
								if (C) {
									if (!C.collapsed) {
										...
										while (1) {
											...
											if (E == (D.getChildCount ? D.getChildCount() : D.getLength()) && !D.isBlockBoundary())
												C.setStartAfter(D);
											else
												break;
										}
						...
						var z = this._.cache;
						if (z.selectedElement !== undefined)
							return z.selectedElement;
						var A = this, B = e.tryThese(function() {
							...
							if (C) {
								...
								var J = e.extend(G, f.$listItem, f.$tableContent), K = new d.walker(I), L = function(M, N) {
									return function(O, P) {
										if (O.type == 3 && (!e.trim(O.getText()) || O.getParent().data('cke-bookmark')))
											return true;
										var Q;
										if (O.type == 1) {
											Q = O.getName();
											if (Q == 'br' && N && O.equals(O.getParent().getBogus()))
												return true;
											if (P && Q in J || Q in f.$removeEmpty)
												return true;
										}
										M.halted = 1;
										return false;
							...
							for (var F = 2; F && !((D = C.getEnclosedNode()) && D.type == 1 && y[D.getName()] && (E = D)); F--)
								C.shrink(1);
							return E.$;
						});
						...
						var z = this._.cache;
						if (z.selectedText !== undefined)
							return z.selectedText;
						var A = '', B = this.getNative();
						if (this.getType() == 2)
							A = c ? B.createRange().text : B.toString();
						...
						if (c) {
							...
							for (var C = 0; C < z.length; C++) {
								...
								if (I.collapsed && (b.opera || b.gecko && b.version < 10900) && K.type == 1 && !K.getChildCount())
									K.appendText('');
			...
			(function() {
				...
				d.range.prototype.select = c ? function(p) {
					...
					if (A.startContainer.type == 1 && A.startContainer.getName() in o || A.endContainer.type == 1 && A.endContainer.getName() in o)
						A.shrink(1, true);
					...
					if (x) {
						...
						s.insertBefore(w);
						if (r)
							A.document.createText('\ufeff').insertBefore(w);
					}
					A.setStartBefore(w);
					w.remove();
					if (q) {
			...
			(function() {
				...
				a.editor.prototype.createFakeElement = function(r, s, t, u) {
					...
					if (!b.hc)
						x.src = a.getUrl('images/spacer.gif');
					if (t)
						x['data-cke-real-element-type'] = t;
					if (u) {
				...
				a.editor.prototype.createFakeParserElement = function(r, s, t, u) {
					...
					if (!b.hc)
						z.src = a.getUrl('images/spacer.gif');
					if (t)
						z['data-cke-real-element-type'] = t;
					if (u) {
				...
				a.editor.prototype.restoreRealElement = function(r) {
					if (r.data('cke-real-node-type') != 1)
						return null;
				...
				e.extend(k.dialog, {labeledElement: function(u, v, w, x) {
						...
						var z = this._.children = [], A = function() {
							...
							if (v.labelLayout != 'horizontal')
								B.push('<label class="cke_dialog_ui_labeled_label' + C + '" ', ' id="' + y.labelId + '"', y.inputId ? ' for="' + y.inputId + '"' : '', (v.labelStyle ? ' style="' + v.labelStyle + '"' : '') + '>', v.label, '</label>', '<div class="cke_dialog_ui_labeled_content"' + (v.controlStyle ? ' style="' + v.controlStyle + '"' : '') + ' role="presentation">', x.call(this, u, v), '</div>');
							else {
								var D = {type: 'hbox', widths: v.widths, padding: 0, children: [{type: 'html', html: '<label class="cke_dialog_ui_labeled_label' + C + '"' + ' id="' + y.labelId + '"' + ' for="' + y.inputId + '"' + (v.labelStyle ? ' style="' + v.labelStyle + '"' : '') + '>' + e.htmlEncode(v.label) + '</span>'}, {type: 'html', html: '<span class="cke_dialog_ui_labeled_content"' + (v.controlStyle ? ' style="' + v.controlStyle + '"' : '') + '>' + x.call(this, u, v) + '</span>'}]};
						...
						z.rows = v.rows || 5;
						z.cols = v.cols || 20;
						if (typeof v.inputStyle != 'undefined')
							z.style = v.inputStyle;
						var A = function() {
						...
						var y = function() {
							...
							if (v['default'])
								C.checked = 'checked';
							if (typeof z.inputStyle != 'undefined')
						...
						var x = [], y = this, z = function() {
							...
							for (var E = 0; E < v.items.length; E++) {
								...
								t(K);
								t(L);
								if (typeof K.inputStyle != 'undefined')
									K.style = K.inputStyle;
						...
						var y = function() {
							...
							for (var D = 0, E; D < v.items.length && (E = v.items[D]); D++)
								B.push('<option value="', e.htmlEncode(E[1] !== undefined ? E[1] : E[0]).replace(/"/g, '&quot;'), '" /> ', e.htmlEncode(E[0]));
							if (typeof z.inputStyle != 'undefined')
								z.style = z.inputStyle;
			...
			a.themes.add('default', (function() {
				...
				function n(o, p) {
					...
					if (r) {
						...
						if (r.getCustomData('cke_hasshared'))
							t.hide();
						else
							r.setCustomData('cke_hasshared', 1);
						q = t.getChild([0, 0, 0, 0]);
						!o.sharedSpaces && (o.sharedSpaces = {});
						o.sharedSpaces[p] = q;
						o.on('focus', function() {
							for (var u = 0, v, w = r.getChildren(); v = w.getItem(u); u++) {
								if (v.type == 1 && !v.equals(t) && v.hasClass('cke_shared'))
									v.hide();
							}
							t.show();
						});
