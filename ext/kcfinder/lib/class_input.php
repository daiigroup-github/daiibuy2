...
class input
{
	...
	public function __construct()
	{
		...
		$this->magic_quotes_sybase = $this->magic_quotes_sybase ? !in_array(strtolower(trim($this->magic_quotes_sybase)), array(
				'off', 'no', 'false')) : false;
	...
	public function filter($subject)
	{
		if ($this->magic_quotes_gpc)
		{
			if (is_array($subject))
			{
				...
				$subject = $this->magic_quotes_sybase ? str_replace("\\'", "'", $subject) : stripslashes($subject);
		}
